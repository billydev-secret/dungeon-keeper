...
class TestHasGuessRole:
    def test_member_with_role_returns_true(self):
        from bot_modules.cogs.guess_cog import _has_guess_role
    ...
    def test_role_id_zero_fails_closed(self):
        """An unconfigured Guess role (id 0) must deny everyone — never fall open."""
        from bot_modules.cogs.guess_cog import _has_guess_role
        m = FakeMember(roles=[])
        assert _has_guess_role(m, 0) is False
        m_with_roles = FakeMember(roles=[FakeRole(id=42)])
        assert _has_guess_role(m_with_roles, 0) is False


class TestValidateMime:
    def test_jpeg_accepted(self):
        from bot_modules.cogs.guess_cog import _validate_mime
...
class TestValidateSize:
    def test_within_limit(self):
        from bot_modules.cogs.guess_cog import _validate_size
...
class TestValidateDimensions:
    def test_large_enough_image(self):
        from bot_modules.cogs.guess_cog import _validate_dimensions
