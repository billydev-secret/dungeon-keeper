"""Deletion scope modes — media/text scrubs vs the full erasure."""
...
def test_only_full_erasure_clears_account_data() -> None:
    assert clears_account_data(MODE_ALL)
    assert not clears_account_data(MODE_MEDIA)
    assert not clears_account_data(MODE_TEXT)


# ── copy ────────────────────────────────────────────────────────────────


def test_partial_summary_does_not_claim_account_data_was_cleared() -> None:
    out = render_deletion_summary(
        deleted=3, failed=0, replaced=0, keep_messages=True, mode=MODE_MEDIA
    )
    assert "left alone" in out
    # The claim that must not appear: that XP/activity/profile were wiped.
    assert "Server-side data" not in out
    assert "**cleared**" not in out


def test_partial_empty_summary_does_not_claim_account_data_was_cleared() -> None:
    out = render_empty_summary(keep_messages=True, mode=MODE_TEXT)
    assert "Server-side data" not in out
    assert "**cleared**" not in out
    assert "Nothing else was touched" in out


def test_full_erasure_summary_is_unchanged() -> None:
    """The pre-existing copy must not drift — modes are additive."""
    out = render_deletion_summary(
        deleted=5, failed=1, replaced=2, keep_messages=True, mode=MODE_ALL
    )
    assert out == (
        "All done. Here's what was removed:\n"
        "Discord messages deleted: **5**\n"
...
def test_self_erasure_prompt_discloses_the_retained_archive() -> None:
    """/delete_me keeps the server's copy — say so before the click, not after."""
    out = render_confirm_prompt(mode=MODE_ALL, keep_messages=True)
    assert "keeps its own copy" in out
    assert "not from those records" in out


def test_full_admin_erasure_prompt_says_nothing_is_kept() -> None:
    out = render_confirm_prompt(mode=MODE_ALL, keep_messages=False, subject="@ben")
    assert "erased too" in out
    assert "keeps its own copy" not in out


def test_partial_prompt_promises_the_account_is_untouched() -> None:
    out = render_confirm_prompt(mode=MODE_MEDIA, keep_messages=True)
    assert "images & files" in out
    assert "stay exactly as they are" in out
    # A scrub must never describe itself as deleting everything.
    assert "everything" not in out.lower()


def test_button_label_names_the_real_scope() -> None:
    assert confirm_button_label(MODE_ALL) == "Yes, delete everything"
...
def test_confirm_view_actually_relabels_the_rendered_button() -> None:
    ...
    assignment would no-op silently and every scrub would still say "delete
    everything" — the exact thing the label exists to prevent.
    """
    ...
    assert "Yes, delete everything" not in labels


def test_confirm_view_defaults_to_the_full_erasure_label() -> None:
    view = privacy_cog._ConfirmDeleteView(actor_id=1)
    assert "Yes, delete everything" in [c.label for c in view.children]
...
def _wired(monkeypatch):
    ...
    Returns a recorder so a test can assert whether the purge actually ran —
    the helper-level check isn't enough, since the bug worth guarding against
    is _run_deletion calling purge anyway.
    """
    calls: dict[str, list] = {"purge": [], "scanned_with": []}
    ...
    def _fake_purge(conn, guild_id, user_id, *, keep_messages=False):
        calls["purge"].append({"user_id": user_id, "keep_messages": keep_messages})
        return 0
    ...
    monkeypatch.setattr(privacy_cog, "purge_user_data", _fake_purge)
...
async def _run(ctx, mode: str, *, keep_messages: bool = True) -> None:
    guild = MagicMock()
    guild.id = 1
    await privacy_cog._run_deletion(
        ctx, guild, 42, MagicMock(), keep_messages=keep_messages, mode=mode
    )


async def test_media_scrub_never_purges_the_account(_wired) -> None:
    calls, ctx = _wired
    await _run(ctx, MODE_MEDIA)
    assert calls["purge"] == [], "a scrub must not touch XP/activity/profile"


async def test_text_scrub_never_purges_the_account(_wired) -> None:
    calls, ctx = _wired
    await _run(ctx, MODE_TEXT)
    assert calls["purge"] == []


async def test_full_erasure_still_purges(_wired) -> None:
    calls, ctx = _wired
    await _run(ctx, MODE_ALL)
    assert calls["purge"] == [{"user_id": 42, "keep_messages": True}]


async def test_scrub_passes_a_predicate_and_erasure_does_not(_wired) -> None:
