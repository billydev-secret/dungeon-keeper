"""Tests for Guess role-removal -> answer_optout flagging + dropdown filtering."""
...
async def test_member_update_flags_orphaned_rounds_when_role_removed():
    ...
    cfg = GuessConfig(guild_id=GUILD_ID, guess_role_id=GUESS_ROLE_ID)
    with patch("bot_modules.cogs.guess_cog._load_config", return_value=cfg), \
         patch("bot_modules.cogs.guess_cog._do_flag_user_open_rounds_optout", return_value=2) as flag_mock:
        await GuessCog.on_member_update(cog, before, after)  # type: ignore[arg-type]
...
async def test_member_update_noop_when_role_still_held():
    """No-op when the user still has the Guess role after the update."""
    from bot_modules.cogs.guess_cog import GuessCog

    cog = _make_cog()
    guess_role = FakeRole(id=GUESS_ROLE_ID)

    before = FakeMember(id=ANSWER_ID, roles=[guess_role])
    after = FakeMember(id=ANSWER_ID, roles=[guess_role])
    after.guild = MagicMock(id=GUILD_ID)

    cfg = GuessConfig(guild_id=GUILD_ID, guess_role_id=GUESS_ROLE_ID)
    with patch("bot_modules.cogs.guess_cog._load_config", return_value=cfg), \
         patch("bot_modules.cogs.guess_cog._do_flag_user_open_rounds_optout") as flag_mock:
        await GuessCog.on_member_update(cog, before, after)  # type: ignore[arg-type]
