...
from bot_modules.services.guess_models import BoundingBox, GuessRound
...
async def test_correct_first_guess_marks_solved_and_edits_message():
    ...
    with patch("bot_modules.cogs.guess_cog._do_count_user_guesses", return_value=0), \
         patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess"), \
         patch("bot_modules.cogs.guess_cog._do_mark_solved", return_value=(1, 3, 2)), \
...
async def test_correct_guess_attaches_full_image_as_spoiler_and_unlinks(tmp_path):
    ...
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess"), \
         patch("bot_modules.cogs.guess_cog._do_mark_solved", return_value=(1, 1, 1)), \
         patch("bot_modules.cogs.guess_cog._do_set_original_path", side_effect=_capture_set_path), \
         patch.object(type(view._select), "values", new=property(lambda _: [str(2001)])):
        await view._on_select(interaction)
    ...
    assert attachments[0].filename == "SPOILER_guess_full.jpg"
...
async def test_wrong_guess_bumps_counter_and_sends_not_it_message():
    ...
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess"), \
         patch("bot_modules.cogs.guess_cog._do_count_guesses_for_round", return_value=4), \
...
async def test_wrong_guess_on_solved_round_skips_counter_bump():
    ...
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess"), \
         patch("bot_modules.cogs.guess_cog._do_count_guesses_for_round", return_value=8), \
         patch.object(type(view._select), "values", new=property(lambda _: [str(3333)])):
        await view._on_select(interaction)
...
async def test_guess_cap_blocks_after_five_attempts():
    ...
    with patch("bot_modules.cogs.guess_cog._do_count_user_guesses", return_value=5), \
...
async def test_guess_cap_allows_under_limit():
    ...
    round_row = _make_round()
    with patch("bot_modules.cogs.guess_cog._do_count_user_guesses", return_value=4), \
         patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess") as insert_mock, \
         patch("bot_modules.cogs.guess_cog._do_count_guesses_for_round", return_value=5), \
         patch.object(type(view._select), "values", new=property(lambda _: [str(3333)])):
        await view._on_select(interaction)
...
async def test_correct_guess_loses_race_does_not_edit_message():
    ...
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess"), \
         patch("bot_modules.cogs.guess_cog._do_mark_solved", return_value=(0, 5, 3)), \
         patch.object(type(view._select), "values", new=property(lambda _: [str(2001)])):
        await view._on_select(interaction)
...
async def test_on_select_rejects_guess_within_cooldown():
    ...
    from bot_modules.services.guess_models import GuessGuess
    ...
    insert_mock = MagicMock()
    with patch("bot_modules.cogs.guess_cog._do_get_last_guess", return_value=recent_guess), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess", insert_mock), \
         patch.object(type(view._select), "values", new=property(lambda _: [str(2001)])):
        await view._on_select(interaction)
...
async def test_on_select_allows_guess_after_cooldown_expires():
    ...
    old_guess = GuessGuess(
        id=1, round_id=ROUND_ID, guesser_id=9999,
        guessed_user_id=2001, correct=False,
        created_at=_time.time() - 60,  # 60s ago, past 30s cooldown
    )
    round_row = _make_round()
    with patch("bot_modules.cogs.guess_cog._do_get_last_guess", return_value=old_guess), \
         patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess") as insert_mock, \
         patch("bot_modules.cogs.guess_cog._do_mark_solved", return_value=(1, 2, 1)), \
         patch.object(type(view._select), "values", new=property(lambda _: [str(2001)])):
        await view._on_select(interaction)
...
async def test_on_select_cooldown_zero_disables_check():
    ...
    with patch("bot_modules.cogs.guess_cog._do_get_last_guess", get_last_mock), \
         patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row), \
         patch("bot_modules.cogs.guess_cog._do_insert_guess"), \
         patch("bot_modules.cogs.guess_cog._do_mark_solved", return_value=(1, 1, 1)), \
         patch.object(type(view._select), "values", new=property(lambda _: [str(2001)])):
...
async def test_guess_callback_message_mentions_timer():
    ...
    config = GuessConfig(guild_id=9001, guess_role_id=GUESS_ROLE_ID, guess_cooldown_seconds=30)

    with patch("bot_modules.cogs.guess_cog._load_config", return_value=config), \
         patch("bot_modules.cogs.guess_cog._do_load_round", return_value=round_row):
...
async def test_post_persists_original_bytes_and_stores_path(tmp_path, monkeypatch):
    ...
    import bot_modules.cogs.guess_cog as guess_cog
    from bot_modules.cogs.guess_cog import CropEditorView

    monkeypatch.setattr(guess_cog, "_GUESS_ORIG_DIR", tmp_path / "orig")
    ...
    with patch("bot_modules.cogs.guess_cog._do_insert_round", return_value=77), \
         patch("bot_modules.cogs.guess_cog._do_update_round_message"), \
         patch("bot_modules.cogs.guess_cog._do_set_original_path", side_effect=_capture), \
