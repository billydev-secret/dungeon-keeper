"""Tests for /guess round mod-only inspector."""
...
def _make_round() -> GuessRound:
    return GuessRound(
        ...
        answer_id=1001, channel_id=GUESS_CHANNEL_ID, message_id=12345,
...
async def _round(cog: Any, interaction: Any, rid: int) -> None:
    await cog.guess_round.callback(cog, interaction, rid)
...
async def test_round_inspector_returns_embed_for_mod():
    ...
    cog = _make_cog()
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=_make_round()), \
         patch("bot_modules.cogs.guess_cog._do_count_guesses_for_round", return_value=12), \
         patch("bot_modules.cogs.guess_cog._do_count_unique_guessers_for_round", return_value=4):
        await _round(cog, interaction, ROUND_ID)
