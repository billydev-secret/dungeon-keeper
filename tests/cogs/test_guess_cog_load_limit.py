...
from bot_modules.services.guess_repo import insert_round, mark_round_solved
...
async def test_cog_load_caps_total_views(sync_db_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(guess_cog, "_COG_LOAD_VIEW_CAP", 3)
...
async def test_cog_load_passes_current_guess_count_to_view(sync_db_path: Path):
    ...
    from bot_modules.services.guess_repo import insert_guess
...
async def test_cog_load_registers_prompt_view(sync_db_path: Path):
    ...
    prompt_calls = [c for c in add_view.call_args_list if isinstance(c.args[0], GuessPromptView)]
