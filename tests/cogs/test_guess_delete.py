"""Cog-level tests for /guess delete + Post-time safety checks."""
...
async def _delete(cog: Any, interaction: Any, round_id: int) -> None:
    await cog.guess_delete.callback(cog, interaction, round_id)


# ── /guess delete authorization ───────────────────────────────────────────────
...
async def test_delete_allowed_for_submitter():
    ...
    cog = _make_cog()
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=_make_round()), \
         patch("bot_modules.cogs.guess_cog._do_soft_delete_round") as soft_del:
...
async def test_delete_rejects_unknown_round():
    ...
    cog = _make_cog()
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=None), \
         patch("bot_modules.cogs.guess_cog._do_soft_delete_round") as soft_del:
        await _delete(cog, interaction, ROUND_ID)
...
async def test_delete_unlinks_original_file_when_present(tmp_path: Path):
    ...
    with patch("bot_modules.cogs.guess_cog._do_load_round", return_value=rnd), \
         patch("bot_modules.cogs.guess_cog._do_soft_delete_round"):
        await _delete(cog, interaction, ROUND_ID)
...
async def test_post_double_click_inserts_only_once(tmp_path, monkeypatch):
    ...
    with patch("bot_modules.cogs.guess_cog._do_insert_round", return_value=77) as insert_mock, \
         patch("bot_modules.cogs.guess_cog._do_update_round_message"), \
         patch("bot_modules.cogs.guess_cog._do_set_original_path"), \
         patch("bot_modules.cogs.guess_cog._do_set_crop_box"), \
         patch("bot_modules.cogs.guess_cog._do_audit"), \
         patch("bot_modules.cogs.guess_cog.render_crop", return_value=b"\xff\xd8fake"), \
         patch("bot_modules.cogs.guess_cog._repost_prompt", new_callable=AsyncMock):
