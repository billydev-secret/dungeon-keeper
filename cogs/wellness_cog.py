...
class WellnessCog(commands.Cog):
    ...
    async def optout_cmd(self, interaction: discord.Interaction) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
            await interaction.response.send_message("Server only.", ephemeral=True)
        ...
        if user is None:
            await interaction.response.send_message(
                "You're not currently opted in to Wellness Guardian.", ephemeral=True
            )
            return
    ...
    async def pause_cmd(
        self, interaction: discord.Interaction, duration: str | None = None
    ) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
    ...
    @wellness.command(name="resume", description="Pick up where you left off after a pause.")
    async def resume_cmd(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            "▶️ Tracking resumed. Welcome back. 💚", ephemeral=True
        )
    ...
    @cap.command(name="add", description="Create a new message cap.")
    @app_commands.describe(
    ...
    async def cap_add_cmd(
        ...
        if scope.value == "voice":
            await interaction.response.send_message(
                "🎙️ Voice caps are coming in v2 — stay tuned!", ephemeral=True
            )
            return
        ...
        with ctx.open_db() as conn:
            ...
            add_cap(
                ...
                label=label, scope=scope.value, scope_target_id=scope_target_id,
                window=window.value, cap_limit=int(limit), exclude_exempt=exclude_exempt,
            )
    ...
    @cap.command(name="list", description="Show all your active caps with current counts.")
    async def cap_list_cmd(self, interaction: discord.Interaction) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
            return
        ...
        if not caps:
            await interaction.response.send_message(
                "You have no caps set. Add one with `/wellness cap add`.", ephemeral=True
            )
            return
    ...
    @cap.command(name="edit", description="Change the limit on an existing cap.")
    ...
    async def cap_edit_cmd(
        ...
        await interaction.response.send_message(
            f"💚 Cap **{label}** updated to **{new_limit}** per {cap.window}.", ephemeral=True
        )

    @cap.command(name="remove", description="Delete a cap.")
    @app_commands.describe(label="Cap to remove.")
    @app_commands.autocomplete(label=_user_cap_label_autocomplete)
    async def cap_remove_cmd(self, interaction: discord.Interaction, label: str) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
            return
    ...
    @blackout.command(name="add", description="Schedule a custom blackout window.")
    @app_commands.describe(
    ...
    async def blackout_add_cmd(
        ...
        if start_min == end_min:
            await interaction.response.send_message(
                "Start and end can't be identical — try a different end time.", ephemeral=True
            )
            return
        ...
        with ctx.open_db() as conn:
            ...
            if existing:
                await interaction.response.send_message(
                    f"You already have a blackout named **{name}**.", ephemeral=True
                )
                return
            add_blackout(
                conn, guild.id, interaction.user.id,
                name=name, start_minute=start_min, end_minute=end_min, days_mask=days_mask,
            )
    ...
    @blackout.command(name="template", description="Apply a preset blackout template.")
    ...
    async def blackout_template_cmd(
        self, interaction: discord.Interaction, template: app_commands.Choice[str]
    ) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
        ...
        with ctx.open_db() as conn:
            ...
            if existing:
                await interaction.response.send_message(
                    f"You already have a blackout named **{tpl['name']}**.", ephemeral=True
                )
                return
            add_blackout(
                conn, guild.id, interaction.user.id,
                name=tpl["name"], start_minute=tpl["start_minute"],
                end_minute=tpl["end_minute"], days_mask=tpl["days_mask"],
            )
        await interaction.response.send_message(
            f"🌙 Applied **{tpl['name']}** — "
    ...
    @blackout.command(name="list", description="Show your blackouts.")
    async def blackout_list_cmd(self, interaction: discord.Interaction) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
            return
    ...
    @blackout.command(name="toggle", description="Enable or disable a blackout.")
    ...
    async def blackout_toggle_cmd(
        self, interaction: discord.Interaction, name: str, enabled: bool
    ) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
        ...
        await interaction.response.send_message(
            f"🌙 Blackout **{name}** is now **{state}**.", ephemeral=True
        )

    @blackout.command(name="remove", description="Delete a blackout.")
    ...
    @app_commands.describe(message=f"New away message (max {AWAY_MESSAGE_MAX} chars).")
    async def away_set_cmd(self, interaction: discord.Interaction, message: str) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
            return
        ...
        with ctx.open_db() as conn:
            update_away_message(
                conn, guild.id, interaction.user.id, enabled=user.away_enabled, message=message
            )
    ...
    async def away_preview_cmd(self, interaction: discord.Interaction) -> None:
        ctx = self.ctx
        user = _require_active_user(ctx, interaction)
        if user is None:
            await interaction.response.send_message(
    ...
    async def partner_request_cmd(
        ...
        if guild is None:
            return
        if member.bot:
            await interaction.response.send_message("Bots can't be partners. 💚", ephemeral=True)
            return
    ...
    @partner.command(name="list", description="Show your accountability partners.")
    async def partner_list_cmd(self, interaction: discord.Interaction) -> None:
        ctx = self.ctx
        guild = interaction.guild
        if guild is None:
            return
        ...
        if accepted_lines:
            embed.add_field(name="Active partnerships", value="\n".join(accepted_lines), inline=False)
        if pending_lines:
    ...
    @partner.command(name="dissolve", description="End a partnership or cancel a pending request.")
    ...
    async def partner_dissolve_cmd(
        ...
        try:
            ...
            await interaction.response.send_message(
                "Pick a partnership from the autocomplete suggestions.", ephemeral=True
            )
            return
        with ctx.open_db() as conn:
