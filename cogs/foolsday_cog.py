...
from services.foolsday_service import (
    active_user_ids as _active_user_ids,
    add_exclusion as _add_exclusion,
    clear_names as _clear_names,
    derangement as _derangement,
    excluded_user_ids as _excluded_user_ids,
    init_tables as _init_table,
    load_names as _load_names,
    remove_exclusion as _remove_exclusion,
    save_names as _save_names,
)
...
# ---------------------------------------------------------------------------
# Repair view classes (module-level so they can be referenced by type hints)
# ---------------------------------------------------------------------------
...
class FoolsdayCog(commands.Cog):
    ...
    async def foolsday_exclude(
        ...
        if (
            ...
            and not self.ctx.is_mod(interaction)
        ):
            await interaction.response.send_message(
    ...
    async def foolsday_join(
        ...
        with self.ctx.open_db() as conn:
            ...
            _remove_exclusion(conn, guild.id, target.id)
    ...
    async def foolsday_include(
        ...
        with self.ctx.open_db() as conn:
            _init_table(conn)
            _remove_exclusion(conn, guild.id, user.id)
    ...
    @app_commands.default_permissions(manage_guild=True)
    async def foolsday_exclusions(self, interaction: discord.Interaction) -> None:
        if not self.ctx.is_mod(interaction):
            await interaction.response.send_message(
                "You don't have permission to use this command.", ephemeral=True
            )
    ...
    @app_commands.default_permissions(manage_guild=True)
    async def foolsday_repair(self, interaction: discord.Interaction) -> None:
        if not self.ctx.is_mod(interaction):
            await interaction.response.send_message(
                "You don't have permission to use this command.",
                ephemeral=True,
...
async def setup(bot: Bot) -> None:
    await bot.add_cog(FoolsdayCog(bot, bot.ctx))
