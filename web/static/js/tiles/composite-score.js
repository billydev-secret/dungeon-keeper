...
export function renderTile(el, data) {
  ...
  const gauge = `<svg viewBox="0 0 200 110" width="200" height="110" style="display:block;margin:0 auto;">
    ...
    <text x="100" y="90" text-anchor="middle" fill="var(--text)" font-size="32" font-weight="bold">${score}</text>
    <text x="100" y="108" text-anchor="middle" fill="var(--text-dim)" font-size="12">/100</text>
  </svg>`;

  // Dimension mini bars
  const dims = (data.dimensions || []).map(d => {
    const color = d.score >= 60 ? "var(--success)" : d.score >= 40 ? "var(--warning)" : "var(--danger)";
