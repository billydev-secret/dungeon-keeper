...
**This script does not use the live phrase rule, and cannot.** Mention Awards
matches a trigger phrase against message content read live off the gateway;
message content is never stored, so there is nothing to re-match over history.
...
watcher runs on the *phrase* rule. That seam is deliberate and is the reason
this is a one-off script rather than a mode of the feature.
...
def _announcements(conn: sqlite3.Connection, guild_id: int, channel_id: int):
    ...
    rows = conn.execute(
        ...
        SELECT m.message_id, m.ts, m.author_id
          ...
          LEFT JOIN games_external_payouts p
                 ON p.message_id = m.message_id AND p.kind = ?
         ...
         ORDER BY m.ts
        """,
        (PAYOUT_KIND, guild_id, channel_id, *CARD_MEDIA_KINDS),
    ).fetchall()
    ...
    for message_id, ts, author_id in rows:
        ...
        if len(mentions) != 1:
            continue
        member_id = next(iter(mentions))
        if member_id == int(author_id):
            continue  # self-nomination, as the live matcher rejects
        yield int(message_id), float(ts), member_id, int(author_id)


def _name(conn: sqlite3.Connection, guild_id: int, user_id: int) -> str:
    row = conn.execute(
    ...
    if not row:
        return str(user_id)
    return str(row[0] or row[1] or user_id)
...
def main() -> int:
    ...
    with open_db(args.db) as conn:
        ...
        for message_id, ts, member_id, announcer_id in _announcements(
            conn, args.guild, args.channel
        ):
            day = local_day_for(ts, offset)
            log.info(
                "  %s  %-24s  named by %s",
                day, _name(conn, args.guild, member_id),
                _name(conn, args.guild, announcer_id),
            )
            ...
            if args.apply:
                ...
                cur = conn.execute(
                    ...
                    (message_id, args.guild, PAYOUT_KIND),
                )
                if cur.rowcount == 0:
                    continue  # raced/duplicate — already paid

                apply_credit(
                    conn, args.guild, member_id, args.amount, "mention_award",
                    meta={"backfill": "pre-watcher", "announced_by": announcer_id},
                    booster=False,
                )
                # The turn's own local day, so a closed board stays closed.
                fire_trigger_quests(
                    conn, settings, args.guild, "mention_award", member_id,
                    local_day=day,
                    occurrence=f"mention_award:{message_id}",
                    booster=False,
                )
        ...
        for uid, coins in per_user.most_common():
            log.info(
                ...
                verb, _name(conn, args.guild, uid), turns[uid], coins,
            )
