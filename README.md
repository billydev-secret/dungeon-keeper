...
- **The Golden Meadow Casino** — House gambling in one admin-configured channel, entirely button-driven (no commands): coinflip, slots, blackjack with double-down, channel-wide roulette rounds, an animated six-critter derby with fixed odds, commission-free punto-banco baccarat, three-dice sic bo, casino war with go-to-war ties, and quick-pick keno with a generosity-tuned paytable. Solo games play out in **private in-place tables** (ephemeral messages), so heavy play never scrolls the channel; the hub panel's **floor ticker** shows recent action, and wins past a dashboard-set threshold — plus every jackpot — get a public broadcast. Fixed, RTP-tested paytables give the house a small edge; per-member daily wager caps and min/max bets are set on the dashboard's Casino page. A loss-fed **progressive jackpot** rides the slots, big bets get animated reveals, and streaks/biggest-wins feed `/bank wallet` stats plus a weekly leaderboard block. The casino's name is per-guild branding (Config → Branding), and its flavor copy is name-agnostic so a rename doesn't leave stray "meadow" text behind. `docs/casino_spec.md` covers it.
...
- **Server todo** — Mods add tasks to a shared list with `/todo`, from a live todo board the bot keeps at the bottom of a channel (with Add and Complete buttons), or from the dashboard. Recurring entries — "post QOTD" daily, "photo challenge prompt" weekly — drop reminder tasks onto the list on a schedule; the board updates itself as tasks come and go.
...
- **`/ask`** — **Billy-bot**, an AI helper that answers "how do I use X" questions in plain language, grounded in the server guide (so it won't invent commands). Optionally (admin toggle, off by default) it also uses live server context — channel topics, pins, announcements, dashboard docs, and (for admins) the server's saved settings across features — scoped to what the asker can *see* and tailored to what they can *do*, linking to channels and the dashboard when helpful. Admin askers get on-demand settings lookup (tool use, not an inline dump), a **setup-gap review** ("what am I missing?" — which features are unconfigured, or wired up but switched off), and can ask Billy-bot to *change* a setting or *stand up a feature from scratch* — applied only via permission-checked Apply buttons on the reply, never by the model directly. Proposals are limited to a vetted registry of settings, in three tiers: ordinary settings for any admin, access- and authority-granting ones (role grants, jailed role, Q&A markers, whisper access) for full server administrators only, and a short never-writable list (`admin_role_ids`, `mod_role_ids`, `message_storage_level`) that no permission level can reach. Role grants (NSFW, Denizen, Veteran, custom) are editable too — pointing a grant at a different role, its log/announce channels, its message — though creating a new grant stays a dashboard action. The same review is pushed, not just pulled: admins get a **Suggested Setup** card on the dashboard home listing the best few features to set up next, cheapest win first. Model + toggle live under Config → Billy-bot, with the model set per audience — members get Haiku, mods/admins get Sonnet (staff asks are the ones that run the settings tools). Replies privately in Discord; also an "Ask Billy-bot" box on every dashboard Help page.
...
- `/games track watch` / `status` / `enable` / `disable` / `sample` — (mod) Track another game bot's results (e.g. Gamebot's CAH/Connect 4, Cat Bot) and pay economy rewards for wins
...
- `/penpals join` / `/penpals leave` — Get a pen pal (matched instantly or once a day at 8am ET, per the dashboard's Pairing Mode setting) or exit the pool; also `status`, `block` (never-match list), `new-question`, `end`, plus mod `pair <user1> <user2>` (both members must already be in the pool) and `round`
