...
`compute_sentiment` still returns the composite fields and this panel still
reads them for its scored-message count, but the home page's narrow
`health-sentiment` widget that rendered them was **removed** on the owner's
call in 2026-09 (todo #185), with `tiles/sentiment.js` and the `sentiment`
branch of the tiles endpoint. An earlier note here argued for keeping it
because removal drops it from saved home layouts: that consequence is real and
accepted — `getLayout()` silently discards widget ids it no longer knows, so an
affected layout loses the box and keeps everything else, with no error. The
wide `health-sentiment-feed` widget was relabelled to **Flagged Messages** to
match.

That removal took the only writer **and** the only reader of the `sentiment`
and `sentiment_outliers` cache keys with it. `set_cached` only ever
`INSERT OR REPLACE`s and `get_cached` treats a past-TTL row as stale without
deleting it, so whatever rows prod holds under those keys — up to four
100-character message excerpts and their author ids apiece — now persist with
no purpose and no reader. `health_metrics_cache` has no subject-id column, so
the privacy-register coverage test cannot see them. Clearing them is a write to
the production database and therefore the owner's to press; this branch did not
do it.
