...
- **Live Auction (built, sink — migration 123, plan
  ...
  **ascending and open**: each bid must beat the standing high by
  ...
  exists until a mod opens one. The four `auction_*` guard-rails are set under
  **Live Auctions** on the **Pricing** page (2026-08-29 — they were enforced on
  every bid from the start but reachable only by writing the config row by
  hand); the route bounds each where the service floors it, so a value that
  would be silently clamped is refused rather than saved. Card renderer in  `economy/auction_views.py`, money in `economy_auction_service.py`.
...
| `voice_room_host` | member's Voice Control room reaches 2+ non-bot guests with the owner present (once per room lifetime, in-memory set; restarts forgive but the occurrence key still blocks a same-room re-pay) | `voice_master_cog._handle_joined_tracked` | `voice_room_host:<channel_id>` |
...
goal now — and its leftover `quest_board_monthly` dial, which sized nothing and
appeared on no panel, was dropped on 2026-08-29.) How many is
...
*opens* on the store has no path left to shorten. A guild selling nothing of
its own gets a one-page shop with no arrows, identical to what it always had.
The channel panel keeps the 8-row `_items_block` teaser — it is a poster that
cannot page — and is the only surface still rendering perks and store in one
embed; that is why the "Weekly rentals" header and "Prices are per week" footer
narrow to "The server store, plus weekly perk rentals" / "Perk prices" exactly
when `items` is passed. Private rooms stay **Stage 6**
...
| Private text room | — | §8 (Stage 6), design-only. The `price_text_room` setting was **dropped 2026-08-29**: nothing could be rented with it, so its only readers were the pricing hints and the Statistics affordability card, which advertised a room nobody could buy. Stage 6 prices itself when it is built |
| Private voice room | — | §8 (Stage 6), design-only — `price_voice_room` dropped alongside it, same date, same reason |
| Custom shop item | admin-set, per item | **Custom shop items** (migration 179, `docs/plans/economy-shop-items.md`). Admin-defined on the Shop & Perks page (the order queue lives on **Approvals**) rather than compiled into `perks.py`; rendered as the shop's own section, absent entirely in a guild with none. Two axes per item: `kind` `role` (grant a role automatically) or `manual` (staff to-do), and `billing` `once` or `weekly` (a real `econ_rentals` row, perk `custom_item`, tagged `catalog_item_id`). Optional stock, per-member limit, availability window, staff-facing description and display order — all six editable on the item's row since 2026-08-29 (the window, description and order were accepted by the API and enforced by the shop, but the editor rendered no input for them, so a limited-time item could not be set up from the dashboard at all). A manual purchase escrows the price (`shop_item` kind) and spawns a row on the **existing mod todo board** (`todos.purchase_id`); ticking it off delivers the order, riding `complete_todo`'s guarded UPDATE for exactly-once. Refusing refunds (`shop_item_refund`, `refunded_at` predicate) and closes the todo as **missed**, never as done. Unresolved orders refund after `shop_item_expire_days` (14, set under **Custom Shop Items** on the Pricing page since 2026-08-29 — the hourly sweep read it all along, but every sibling review window was editable and this one was not) |
...
select cap). That trim runs in `sort_order, id` order, so each row's **Order**
box on Shop & Perks decides which icons survive it — an input the row editor
lacked until 2026-08-29, which left every icon at 0 and the cut falling to
insertion order with no way for an admin to change it. Picking Custom rents the flat perk when unowned, re-tags a live
...
- **Discord is the entire v1 member surface** (decided — the member-facing dashboard
  ...
  the guide, leaderboard, and shop panels are posted from Economy → Settings → Post to Discord)
  plus `/qotd post` [mod]
  ...
  - **`/bank pay @member amount`** — transfer (§5); **`/bank store`** — the same
    shop by a shorter name, opened on its store page (and refusing by name
    where the guild stocks nothing); **`/bank shop`** — one ephemeral
    paged panel that both browses and configures. The listing is an aligned code-cell
...
- **Shop panel (shipped):** posted from **Economy → Settings → Post to Discord on the dashboard (replaced the slash command 2026-07-28)** [admin] —
  ...
  a **single 🛍️ Open Shop button** (`ShopPanelView`, static custom_id
  `econ_shop_open`, re-registered in `cog_load` — the static-custom_id view
  pattern the economy panel's own buttons use). Since 2026-08-28 the panel
  embed also lists the guild's store (read with no `user_id`, so nothing is
  ticked as owned) as an 8-row teaser pointing back at Open Shop. A second
  **🎁 Server Store** button was built here and removed before merge — the shop
  now opens on the store, so the shortcut had nothing to shorten.
...
- **The economy panel (shipped, live):** posted from Economy → Settings → Post
  ...
  names, per the 2026-07-18 ticker decision).
  **Goals and the board share one heading** (`BOARD_HEADING`, "📋 Quest board
  & community goals", 2026-07-27): two adjacent headings read as two competing
  boards when both answer "what is there to do". Goals lead, the cadence
...
- **Voice rooms:** Voice Control's owner registry, capped-FIFO trust/block lists
