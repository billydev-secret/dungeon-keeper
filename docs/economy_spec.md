...
- **Live Auction (built, sink — migration 123, plan
  ...
  **ascending and open**: each bid must beat the standing high by
  ...
  exists until a mod opens one. The four `auction_*` guard-rails (opening bid,
  minimum raise, anti-snipe window, longest auction) are set under **Live
  Auctions** on the **Pricing** page (2026-08-29 — before that they were
  enforced on every auction but settable only by hand-editing the config
  table); a change applies to the next auction, since the limits are snapshotted
  onto the auction row when it starts. Card renderer in
