...
> *[a banner card: the question set over the server icon, headed "Ember"]*
...
- The card is the same renderer behind `/quote banner` and the QOTD card (midnight theme, server icon as the graded background), so a revive reads as the same object as the rest of the server's cards. Its heading is the persona — "Ember" — not the feature name: QOTD is a thing members know by name, but a revive works better as an organic nudge than as a bot announcing its own machinery.
