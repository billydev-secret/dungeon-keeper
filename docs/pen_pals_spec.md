...
| `/penpals leave` | Slash | Everyone (server only) | Leave the pool **and stay out** — works mid-chat too, and holds until you rejoin |
...
**`pen_pals_pool_events`** (migration 160) — append-only history of pool movement: `guild_id`, `user_id`, `at`, `action` (`join` / `leave` / `skip`), `reason` (`panel`, `command`, `dm`, `requeue_expired`, `requeue_abnormal`, `backfill`, `matched`, `departed`, `inactive`, `opted_out`). The pool table is current-state only, which made the two questions you actually ask of a matchmaking pool — "were these people matched, or did they drop out?" and "how long has this been stuck?" — unanswerable from the data; the 2026-08-14 stall had to be reconstructed from session timestamps. `skip` is the odd one out: nothing moved, and the row records that a closing session deliberately did *not* re-pool someone — because they never posted (`inactive`), had left the guild (`departed`), or have opted out (`opted_out`, migration 173). Metadata only, and cleared by `purge_user_data` — see `docs/data_register.md`. Surfaced on the dashboard as **Pool Activity** (`GET /config/pen-pals/pool-events`, moderator).

**`pen_pals_optouts`** (migration 173) — one row per member who has asked not to be matched: `guild_id`, `user_id`, `at`. Presence is the whole state; `at` records when they *first* asked and is not refreshed by pressing Leave again. Kept as its own table rather than a column on `pen_pals_pool` precisely because the pool row is the thing that keeps being deleted. Cleared by joining, and by nothing else: it is **preserved through a full erasure**, unlike every other Pen Pals table that keys on the member. An erasure preserves `pen_pals_pool` and `pen_pals_sessions` too, so a purged member can still be pooled or in a session that re-pools them at expiry — dropping the opt-out would silently re-enrol exactly the person who asked to be left out. Same Art 17(3) footing as a no-contact order; disclosed in full by the access export. See `docs/data_register.md`. Surfaced on the dashboard as **Opted Out** under Pool Activity (same moderator endpoint), read-only.
