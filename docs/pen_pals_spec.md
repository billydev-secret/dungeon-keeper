...
**`pen_pals_pool_events`** (migration 160) — append-only history of pool movement: `guild_id`, `user_id`, `at`, `action` (`join` / `leave` / `skip`), `reason` (`panel`, `command`, `dm`, `requeue_expired`, `requeue_abnormal`, `backfill`, `matched`, `departed`, `inactive`). The pool table is current-state only, which made the two questions you actually ask of a matchmaking pool — "were these people matched, or did they drop out?" and "how long has this been stuck?" — unanswerable from the data; the 2026-08-14 stall had to be reconstructed from session timestamps. `skip`/`inactive` is the odd one out: nothing moved, and the row records that an expiring session deliberately did *not* re-pool someone. Metadata only, and cleared by `purge_user_data` — see `docs/data_register.md`. Surfaced on the dashboard as **Pool Activity** (`GET /config/pen-pals/pool-events`, moderator).
