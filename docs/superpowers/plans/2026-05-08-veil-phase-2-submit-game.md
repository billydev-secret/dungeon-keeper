# Veil Phase 2: Submit Command + Game Flow

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement `/veil submit` so a Veil-roled member can post a cropped image as a guessing-game round, and any server member can guess the answer via a dropdown.

**Architecture:** All Discord interaction code lives in `cogs/veil_cog.py` (expanded from the Phase 1 stub) alongside the three View classes (SubmitPreviewView, GameView, GuessSelectView). The DB layer (`services/veil_repo.py`) and pipeline (`services/veil_pipeline.py`) are already built; this plan only wires them to Discord. GameView is registered as a persistent view so Guess buttons survive bot restarts.

**Tech Stack:** discord.py 2.x `app_commands.Group`, `discord.ui.View`, `asyncio.to_thread` for sync DB/pipeline calls, PIL for dimension check, `services/veil_pipeline.py`, `services/veil_repo.py`, `tests/fakes.py` + `@pytest.mark.asyncio` for cog tests.
...
| File | Status | Purpose |
|------|--------|---------|
| `cogs/veil_cog.py` | Modify | Module-level helpers, all three Views, VeilCog with `/veil submit`, startup re-registration |
| `services/veil_repo.py` | Modify | Add `get_all_active_round_ids` for startup |
| `tests/unit/test_veil_helpers.py` | Create | Unit tests for pure validation helpers |
| `tests/unit/test_veil_repo.py` | Modify | Add test for `get_all_active_round_ids` |
| `tests/cogs/test_veil_submit.py` | Create | Cog-level tests for submit command |
| `tests/cogs/test_veil_guess.py` | Create | Cog-level tests for guess + solve flow |
...
- **`services/veil_pipeline.py`**: `run_pipeline(image_path, image_bytes, difficulty, *, candidate_count, cache_dir) -> PipelineResult` and `run_reroll(image_bytes, existing_crops, *, jpeg_quality) -> bytes`
- **`services/veil_repo.py`**: `get_veil_config`, `insert_round`, `update_round_message`, `mark_round_solved`, `set_round_reroll_count`, `count_guesses_for_round`, `count_unique_guessers_for_round`, `insert_guess`, `get_round`
- **`services/veil_models.py`**: `VeilConfig`, `VeilRound`, `PipelineResult`, `BoundingBox`
- **`cogs/veil_cog.py`**: stub `VeilCog` with a placeholder `veil_status` command
...
The DB schema (`veil_rounds`, `veil_guesses`, `veil_optins`) is migrated and ready.
...
**Files:**
- Modify: `cogs/veil_cog.py` (add helpers above `VeilCog` class, replacing the stub)
- Create: `tests/unit/test_veil_helpers.py`
...
```
pytest tests/unit/test_veil_helpers.py -v
```
Expected: `ImportError` or `ModuleNotFoundError` — the helpers don't exist yet.

- [ ] **Step 3: Replace `cogs/veil_cog.py` with the implementation**
...
def _has_veil_role(member: discord.Member, veil_role_id: int) -> bool:
...
# ── Views (defined before VeilCog so VeilCog can reference them) ──────────────
...
```bash
git add cogs/veil_cog.py tests/unit/test_veil_helpers.py
git commit -m "feat(veil): validation helpers + command group scaffold"
```

---

## Task 2: `get_all_active_round_ids` in veil_repo + test

**Files:**
- Modify: `services/veil_repo.py` (add function at end)
- Modify: `tests/unit/test_veil_repo.py` (add two tests)

- [ ] **Step 1: Write failing tests**

Add to `tests/unit/test_veil_repo.py`:

```python
from services.veil_repo import get_all_active_round_ids
...
Note: `mark_round_solved` and `soft_delete_round` are already imported in `test_veil_repo.py`; add them to the import line if not already present.

- [ ] **Step 2: Run to confirm failure**

```
pytest tests/unit/test_veil_repo.py::test_get_all_active_round_ids_returns_empty_when_no_rounds -v
```
Expected: `ImportError` — `get_all_active_round_ids` does not exist yet.

- [ ] **Step 3: Add function to `services/veil_repo.py`**
...
```
pytest tests/unit/test_veil_repo.py -v
```
...
git commit -m "feat(veil): add get_all_active_round_ids for startup re-registration"
```
...
**Files:**
- Create: `tests/cogs/test_veil_submit.py`
...
async def test_submit_rejects_no_veil_role():
    ...
    with patch("cogs.veil_cog._load_config", return_value=_cfg()):
        await cog.veil_submit(interaction, image=_attachment(), allow_reuse=False)
...
Actually: after completing Task 1 (which replaced `veil_cog.py`), these tests should pass. Run now:
...
```bash
git add tests/cogs/test_veil_submit.py
git commit -m "test(veil): submit command validation rejection tests"
```
...
**Files:**
- Modify: `tests/cogs/test_veil_submit.py` (add success path test)

- [ ] **Step 1: Write failing test**

Add to `tests/cogs/test_veil_submit.py`:
...
async def test_submit_success_posts_game_and_sends_preview(sync_db_path: Path):
    ...
    # Set up a fake veil channel on the guild
...
```
pytest tests/cogs/test_veil_submit.py::test_submit_success_posts_game_and_sends_preview -v
```
...
```bash
git add tests/cogs/test_veil_submit.py
git commit -m "test(veil): submit success path integration test"
```
...
**Files:**
- Create: `tests/cogs/test_veil_guess.py`
...
def _make_select_view(
    ...
    return GuessSelectView(bot, round_id, veil_members, game_message)
...
async def test_correct_first_guess_marks_solved_and_edits_message():
    ...
    with patch("cogs.veil_cog._do_load_round", return_value=round_row):
        with patch("cogs.veil_cog._do_insert_guess"):
            with patch("cogs.veil_cog._do_mark_solved", return_value=(3, 2)):
...
async def test_select_is_disabled_after_guess():
    ...
    with patch("cogs.veil_cog._do_load_round", return_value=round_row):
        with patch("cogs.veil_cog._do_insert_guess"):
            with patch("cogs.veil_cog._do_mark_solved", return_value=(1, 1)):
                await view._on_select(interaction)
...
async def test_game_view_rejects_submitter_guessing_own_round():
    ...
    with patch("cogs.veil_cog._load_config"):
        with patch("cogs.veil_cog._do_load_round", return_value=round_row):
...
```
pytest tests/cogs/test_veil_guess.py -v
```
Expected: 9 PASS.
...
```bash
git add tests/cogs/test_veil_guess.py
git commit -m "test(veil): guess flow, solve logic, reroll view tests"
```
...
**Files:**
- Modify: `tests/cogs/test_veil_submit.py` (add `cog_load` test)
...
```
pytest tests/cogs/test_veil_submit.py::test_cog_load_registers_game_views_from_db -v
```
Expected: PASS.
...
```bash
git add tests/cogs/test_veil_submit.py
git commit -m "test(veil): cog_load view re-registration test + suite check"
```
...
- [ ] `/veil submit` with no Veil role → ephemeral rejection
- [ ] `/veil submit` with unconfigured veil_channel_id=0 → ephemeral rejection
- [ ] `/veil submit` with non-image MIME → ephemeral rejection
- [ ] `/veil submit` with oversized file → ephemeral rejection
- [ ] `/veil submit` with small dimensions → ephemeral rejection
- [ ] `/veil submit` with no NudeNet detections → ephemeral "couldn't find crop region"
- [ ] `/veil submit` success → public game message in veil channel + ephemeral preview
- [ ] Re-roll button cycles crops, updates game message, disables at max
- [ ] Guess button on game message → ephemeral dropdown with veil-role members
...
- `/veil optin` / `/veil optout` not implemented (assign Veil role manually for now)
- Re-roll `veil_cache` file is updated to the latest crop but only the first crop's CDN URL is stored — the CDN URL updates are a future task
