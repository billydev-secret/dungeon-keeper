# Guess Prompt Repost After Game Post Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** After a Guess game round is posted to the channel, reposition the sticky channel-bottom prompt so it appears below the new round.

**Architecture:** Add a best-effort `_repost_prompt(...)` call at the end of `SubmitPreviewView._on_post` in `cogs/guess_cog.py`. The function already exists and handles its own errors; the new call mirrors the pattern in `_delayed_repost_prompt`.
...
**Files:**
- Modify: `cogs/guess_cog.py` (end of `SubmitPreviewView._on_post`)
- Test: `tests/cogs/test_guess_submit.py`

- [ ] **Step 1: Write the failing test**

Add this test to `tests/cogs/test_guess_submit.py`:
...
Run: `pytest tests/cogs/test_guess_submit.py::test_on_post_reposts_prompt_after_game_message -v`
...
In `cogs/guess_cog.py`, at the end of `SubmitPreviewView._on_post` — after `await interaction.edit_original_response(...)` — add:
...
- [ ] **Step 5: Run the full guess test suite to check for regressions**

Run: `pytest tests/cogs/test_guess_submit.py tests/cogs/test_guess_prompt.py -v`
...
git commit -m "feat(guess): repost prompt after game round is posted"
```
