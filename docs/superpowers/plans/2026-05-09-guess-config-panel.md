# Guess Config Web Panel — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a web dashboard config panel so all Guess NSFW-game settings can be managed from the Config section, instead of only through the `/guess setup` Discord command.

**Architecture:** Add a `"guess"` block to the existing `GET /api/config` response and a new `PUT /api/config/guess` endpoint in `web/routes/config.py` (same pattern as confessions, spoiler, starboard). A new JS panel `config-guess.js` reads `config.guess` from `loadConfig()` and submits to the PUT endpoint. One line added to `app.js` registers it in the Config sidebar.

**Tech Stack:** Python / FastAPI / Pydantic (backend), vanilla ES-module JS with DOM manipulation (frontend), SQLite via `services/guess_repo.py`
...
| File | Action |
|------|--------|
| `web/routes/config.py` | Add `_guess_section()` helper + `"guess"` key in GET; add `GuessConfigUpdate` model + `PUT /config/guess` endpoint |
| `tests/web/test_config_routes.py` | Add tests for guess GET section + PUT endpoint |
| `web/static/js/panels/config-guess.js` | Create: single flat form panel using DOM manipulation |
| `web/static/js/app.js` | Register panel in Config section |

---

## Task 1: Backend GET — guess section in `/api/config`
...
```
pytest tests/web/test_config_routes.py::test_get_config_includes_guess_section -v
```

Expected: `FAILED` — `KeyError: 'guess'`
...
```python
from services.guess_repo import get_guess_config as _get_guess_config
```

- [ ] **Step 4: Add `_guess_section` helper to `web/routes/config.py`**
...
def _guess_section(conn, guild_id: int) -> dict:
    vc = _get_guess_config(conn, guild_id)
    return {
        "channel_id": str(vc.guess_channel_id),
        "role_id": str(vc.guess_role_id),
...
- [ ] **Step 5: Wire `_guess_section` into the GET /api/config response**
...
git commit -m "feat(guess): expose guess config in GET /api/config"
```

---

## Task 2: Backend PUT — `/api/config/guess` endpoint
...
def test_update_guess_reuse_disabled(authed_client, fake_ctx):
    resp = authed_client.put("/api/config/guess", json={"reuse_enabled": False})
    assert resp.status_code == 200
    assert resp.json()["ok"] is True
    with open_db(fake_ctx.db_path) as conn:
        from db_utils import get_config_value
        val = get_config_value(conn, "guess_reuse_enabled", "1", fake_ctx.guild_id)
    assert val == "0"
```
...
```
pytest tests/web/test_config_routes.py::test_update_guess_channel tests/web/test_config_routes.py::test_update_guess_crop_difficulty_hard tests/web/test_config_routes.py::test_update_guess_invalid_difficulty_returns_error tests/web/test_config_routes.py::test_update_guess_reuse_disabled -v
```

Expected: all `FAILED` — 404 or similar (endpoint doesn't exist yet)

- [ ] **Step 3: Add `GuessConfigUpdate` model and `PUT /config/guess` to `web/routes/config.py`**
...
async def update_guess_config(
    ...
    def _q():
        from services.guess_repo import set_guess_config_value
        ...
        with ctx.open_db() as conn:
            ...
            if body.reuse_enabled is not None:
                set_guess_config_value(conn, guild_id, "guess_reuse_enabled", "1" if body.reuse_enabled else "0")
            if body.reuse_quiet_hours is not None:
                set_guess_config_value(conn, guild_id, "guess_reuse_quiet_hours", str(body.reuse_quiet_hours))
            if body.reuse_min_age_days is not None:
                set_guess_config_value(conn, guild_id, "guess_reuse_min_age_days", str(body.reuse_min_age_days))
            if body.reuse_min_post_interval_hours is not None:
                set_guess_config_value(conn, guild_id, "guess_reuse_min_post_interval_hours", str(body.reuse_min_post_interval_hours))
        return {"ok": True}
...
```bash
git add web/routes/config.py tests/web/test_config_routes.py
git commit -m "feat(guess): add PUT /api/config/guess endpoint"
```

---

## Task 3: Frontend — `config-guess.js` panel + register in `app.js`

**Files:**
- Create: `web/static/js/panels/config-guess.js`
- Modify: `web/static/js/app.js`

No automated tests for JS panels in this codebase. Verify manually after the server is running (see Step 3).

- [ ] **Step 1: Create `web/static/js/panels/config-guess.js`**
...
In `app.js`, find the Config section (search for `id: "config-ai"`). Add a "Guess" entry immediately after it:
...
```javascript
      { id: "config-ai",          label: "AI Commands",      module: "./panels/config-ai.js" },
      { id: "config-guess",        label: "Guess",             module: "./panels/config-guess.js" },
```
...
```bash
git add web/static/js/panels/config-guess.js web/static/js/app.js
git commit -m "feat(guess): add Guess config panel to web dashboard"
```
