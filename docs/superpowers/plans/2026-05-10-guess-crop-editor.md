# Guess Crop Editor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the pipeline-controlled crop selection in `/guess submit` with an interactive Discord UI that shows the full image with a moveable/zoomable crop box, using a 3×3 button grid.

**Architecture:** Two new pure functions (`move_crop_box`, `zoom_crop_box`) live in `services/guess_pipeline.py` alongside the existing geometry helpers. A new `render_crop_editor` function in `services/guess_crop_renderer.py` renders the full image scaled to 1280px with a red rectangle overlay. `CropEditorView` in `cogs/guess_cog.py` replaces `SubmitPreviewView` — on each button press it updates `self.crop_box`, calls `render_crop_editor` via `asyncio.to_thread`, and edits the ephemeral message. On ✓ it calls `render_crop` to produce the final crop and posts to the game channel.
...
| File | Change |
|---|---|
| `services/guess_pipeline.py` | Add `move_crop_box`, `zoom_crop_box` |
| `services/guess_crop_renderer.py` | Add `render_crop_editor` |
| `cogs/guess_cog.py` | Replace `SubmitPreviewView` with `CropEditorView`; update imports; update `guess_submit` |
| `tests/unit/test_guess_pipeline.py` | Add geometry tests (existing file) |
| `tests/unit/test_guess_crop_renderer.py` | Add overlay renderer tests (existing file) |
| `tests/cogs/test_guess_crop_editor.py` | New — CropEditorView button callback tests |
| `tests/cogs/test_guess_submit.py` | Update two affected tests |
...
**Files:**
- Modify: `services/guess_pipeline.py` (append two functions after `enforce_min_size`)
- Modify: `tests/unit/test_guess_pipeline.py` (append tests at the end)

- [ ] **Step 1: Write the failing tests**

Append to `tests/unit/test_guess_pipeline.py`:
...
```
pytest tests/unit/test_guess_pipeline.py -k "move_crop_box or zoom_crop_box" -v
```
...
In `services/guess_pipeline.py`, append after the `enforce_min_size` function (after line 208):
...
```
pytest tests/unit/test_guess_pipeline.py -v
```
...
git commit -m "feat(guess): add move_crop_box and zoom_crop_box geometry helpers"
```
...
**Files:**
- Modify: `services/guess_crop_renderer.py` (append function)
- Modify: `tests/unit/test_guess_crop_renderer.py` (append tests)

- [ ] **Step 1: Write the failing tests**

Append to `tests/unit/test_guess_crop_renderer.py`:

```python
# ── render_crop_editor ────────────────────────────────────────────────────────

def test_render_crop_editor_returns_jpeg_bytes():
    from services.guess_crop_renderer import render_crop_editor
...
```
pytest tests/unit/test_guess_crop_renderer.py -k "render_crop_editor" -v
```
...
In `services/guess_crop_renderer.py`, append after `render_crop`:
...
```
pytest tests/unit/test_guess_crop_renderer.py -v
```
...
```
git add services/guess_crop_renderer.py tests/unit/test_guess_crop_renderer.py
git commit -m "feat(guess): add render_crop_editor overlay renderer"
```
...
**Files:**
- Create: `tests/cogs/test_guess_crop_editor.py`
- Modify: `cogs/guess_cog.py` (add class, update imports, add constants)
...
def _make_view(
    ...
    from cogs.guess_cog import CropEditorView
...
async def test_up_moves_crop_upward():
    ...
    with patch("cogs.guess_cog.render_crop_editor", return_value=b"\xff\xd8fake"):
...
```
pytest tests/cogs/test_guess_crop_editor.py -v
```
...
- [ ] **Step 3: Update imports in `cogs/guess_cog.py`**

Replace the current single import from `services.guess_pipeline`:
```python
from services.guess_pipeline import run_pipeline
```
with:
```python
from services.guess_crop_renderer import render_crop, render_crop_editor
from services.guess_pipeline import (
    compute_padded_crop,
    enforce_min_size,
    move_crop_box,
...
Also add `BoundingBox` to the existing `services.guess_models` import:
```python
from services.guess_models import BoundingBox, GuessConfig, GuessGuess, GuessRound
```
...
class CropEditorView(discord.ui.View):
    ...
    async def _on_post(self, interaction: discord.Interaction) -> None:
        ...
        role_ping = f"<@&{self._guess_role_id}>" if self._guess_role_id else None
...
- [ ] **Step 6: Run all guess tests to verify they pass**

```
pytest tests/cogs/test_guess_crop_editor.py tests/unit/test_guess_pipeline.py tests/unit/test_guess_crop_renderer.py -v
```
...
```
git add cogs/guess_cog.py tests/cogs/test_guess_crop_editor.py
git commit -m "feat(guess): add CropEditorView interactive crop editor"
```

---

## Task 4: Wire `guess_submit` to `CropEditorView` and remove `SubmitPreviewView`

**Files:**
- Modify: `cogs/guess_cog.py` (update `guess_submit`, delete `SubmitPreviewView`)
- Modify: `tests/cogs/test_guess_submit.py` (update two affected tests)
...
async def test_on_post_reposts_prompt_after_game_message():
    ...
    with patch("cogs.guess_cog._do_insert_round", return_value=42), \
         patch("cogs.guess_cog._do_update_round_message"), \
         patch("cogs.guess_cog._do_audit"), \
         patch("cogs.guess_cog.render_crop", return_value=b"\xff\xd8fake"), \
         patch("cogs.guess_cog._repost_prompt", new_callable=AsyncMock) as mock_repost:
        await view._on_post(interaction)

    mock_repost.assert_awaited_once_with(bot, fake_channel, GUILD_ID)
```

- [ ] **Step 3: Run the test file to confirm the two tests now fail (expected — `CropEditorView` exists but `guess_submit` still creates `SubmitPreviewView`)**

```
pytest tests/cogs/test_guess_submit.py -v
```

Note the failures — they should be about `SubmitPreviewView` still being used in `guess_submit` (for `test_submit_success_sends_ephemeral_preview`) and the import change (for `test_on_post_reposts_prompt_after_game_message`).

- [ ] **Step 4: Update `guess_submit` in `cogs/guess_cog.py`**

In the `guess_submit` method:
...
- [ ] **Step 6: Run all guess tests**

```
pytest tests/cogs/test_guess_submit.py tests/cogs/test_guess_crop_editor.py tests/unit/test_guess_pipeline.py tests/unit/test_guess_crop_renderer.py -v
```
...
```
git add cogs/guess_cog.py tests/cogs/test_guess_submit.py
git commit -m "feat(guess): replace SubmitPreviewView with interactive CropEditorView"
```
...
- `render_crop_editor` signature matches calls in `_rerender` and `guess_submit`
