# Veil Round Message Chips Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add two disabled chip buttons (`Guesses: N`, `Submitted by ▒▒▒▒▒▒▒`) to the public Veil round message in unsolved state, and bump the counter on every wrong guess.
...
**Spec:** `docs/superpowers/specs/2026-05-10-veil-chips-design.md`
...
| File | Role |
|------|------|
| `cogs/veil_cog.py` | All implementation changes — embed, `GameView`, `_on_select`, `cog_load`. |
| `tests/cogs/test_veil_guess.py` | Update wrong-guess test (now expects an edit); add tests for counter bump and skip cases. |
| `tests/cogs/test_veil_cog_load_limit.py` | Add a test that reconstructed views carry the correct count. |
...
**Files:**
- Modify: `cogs/veil_cog.py` (`GameView` class, `_game_embed`)
- Test: `tests/cogs/test_veil_cog_structure.py` (add a structural test in a fresh `class TestChipRendering`)
...
Append to `tests/cogs/test_veil_cog_structure.py`:
...
If `tests/cogs/test_veil_cog_structure.py` doesn't already import `MagicMock`, add `from unittest.mock import MagicMock` near the top.

- [ ] **Step 2: Run tests to verify they fail**

Run: `pytest tests/cogs/test_veil_cog_structure.py -v -k "chip or no_anonymous"`
...
Replace this in `cogs/veil_cog.py`:
...
Replace the `GameView.__init__` in `cogs/veil_cog.py`:
...
- [ ] **Step 6: Run the full veil test suite to confirm nothing else regressed yet**

Run: `pytest tests/cogs/test_veil_cog_structure.py tests/cogs/test_veil_guess.py tests/cogs/test_veil_submit.py tests/cogs/test_veil_cog_load_limit.py -v`
...
```bash
git add cogs/veil_cog.py tests/cogs/test_veil_cog_structure.py
git commit -m "feat(veil): render chip buttons on unsolved round view"
```
...
**Files:**
- Modify: `cogs/veil_cog.py` (`GuessSelectView._on_select`)
- Test: `tests/cogs/test_veil_guess.py`
...
- [ ] **Step 1: Update the failing wrong-guess test in `tests/cogs/test_veil_guess.py`**
...
Append to `tests/cogs/test_veil_guess.py`:
...
Run: `pytest tests/cogs/test_veil_guess.py::test_wrong_guess_bumps_counter_and_sends_not_it_message tests/cogs/test_veil_guess.py::test_wrong_guess_on_solved_round_skips_counter_bump -v`
...
In `cogs/veil_cog.py`, find the final `else:` branch of `_on_select` (the wrong-guess path):
...
- [ ] **Step 6: Run the full veil test suite — confirm no regressions**

Run: `pytest tests/cogs/test_veil_guess.py tests/cogs/test_veil_submit.py tests/cogs/test_veil_cog_structure.py tests/cogs/test_veil_cog_load_limit.py -v`
...
```bash
git add cogs/veil_cog.py tests/cogs/test_veil_guess.py
git commit -m "feat(veil): bump guess counter chip on wrong guesses"
```
...
**Files:**
- Modify: `cogs/veil_cog.py` (`VeilCog.cog_load`)
- Test: `tests/cogs/test_veil_cog_load_limit.py`
...
Append to `tests/cogs/test_veil_cog_load_limit.py`:
...
Run: `pytest tests/cogs/test_veil_cog_load_limit.py::test_cog_load_passes_current_guess_count_to_view -v`
Expected: FAIL — `"Guesses: 0"` is in labels, `"Guesses: 3"` is not.

- [ ] **Step 3: Update `cog_load` to fetch counts**

In `cogs/veil_cog.py`, replace `VeilCog.cog_load`:
...
Run: `pytest tests/cogs/test_veil_cog_load_limit.py -v`
Expected: all pass (including the existing two).

- [ ] **Step 5: Run the full veil suite once more**

Run: `pytest tests/cogs/test_veil_cog_structure.py tests/cogs/test_veil_guess.py tests/cogs/test_veil_submit.py tests/cogs/test_veil_cog_load_limit.py tests/cogs/test_veil_setup.py tests/cogs/test_veil_delete.py tests/cogs/test_veil_round_inspector.py tests/cogs/test_veil_optout_listener.py -v`
Expected: all pass.

- [ ] **Step 6: Run ruff and pyright**

Run: `ruff check cogs/veil_cog.py tests/cogs/`
Expected: no errors.

Run: `pyright cogs/veil_cog.py`
Expected: no errors.

- [ ] **Step 7: Commit**

```bash
git add cogs/veil_cog.py tests/cogs/test_veil_cog_load_limit.py
git commit -m "feat(veil): restore current guess count on view reconstruction"
```
...
| Disabled chips, U+2592 redaction bar, custom IDs `veil_chip_count:{id}` / `veil_chip_submitter:{id}` | Task 1 (visible in code blocks) |
...
All spec sections accounted for. No placeholder steps. Type signature `guess_count: int = 0` is consistent across `GameView.__init__`, `cog_load`'s constructor call, and `_on_select`'s constructor call. Custom IDs `veil_chip_count:{round_id}` / `veil_chip_submitter:{round_id}` are used identically in code and tests.
