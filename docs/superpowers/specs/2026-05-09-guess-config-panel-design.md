...
Add a web dashboard config panel for the Guess NSFW guessing game. All guess settings currently configured via `/guess setup` (and its defaults) become editable from the Config section of the dashboard.

## Files Changed

| File | Change |
|------|--------|
| `web/routes/config.py` | Add `"guess"` block to `GET /api/config` response; add `PUT /api/config/guess` endpoint |
| `web/static/js/panels/config-guess.js` | New panel — single flat form |
| `web/static/js/app.js` | Register panel in Config section |
| `cogs/guess_cog.py` | No changes — `/guess setup` remains as a Discord-side alternative |

## Architecture

Follows the same pattern as confessions, spoiler, and starboard:

- `GET /api/config` (existing endpoint) gains a `"guess"` key populated by calling `get_guess_config` from `services/guess_repo.py`.
- `PUT /api/config/guess` writes individual keys via the existing `set_guess_config_value`.
...
Accepts the same shape. Validates `crop_difficulty` is one of `easy | medium | hard`. Rejects unknown keys. Writes each field via `set_guess_config_value(conn, guild_id, key, value)`.

## Panel Layout

Single flat form in the Config section, labelled "Guess":
...
- No changes to `/guess setup` Discord command — it continues to work as before.
