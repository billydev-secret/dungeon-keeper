...
Add two disabled "chip" buttons to the public Veil round message: a running guess counter and a censored "Submitted by" indicator. The chips replace the current `Submitted by an anonymous member` embed description and give viewers visible feedback on round activity before they ever click Guess.
...
| File | Change |
|------|--------|
| `cogs/veil_cog.py` | `_game_embed` drops the description; `GameView` (unsolved only) adds two disabled chip buttons; `_on_select` edits the public message to bump the counter after every guess (correct or wrong) |
| `tests/cogs/test_veil_guess.py` | Add coverage: counter increments on every guess; chips absent in solved-state view |
...
Disabled buttons in a persistent View need custom IDs for component routing, but since chips have no callbacks, the IDs only need to be unique within the View. Use static IDs `veil_chip_count:{round_id}` and `veil_chip_submitter:{round_id}`.
...
The persistent-view registration done at cog load (`bot.add_view(GameView(..., solved=False))`) is unaffected — Discord routes button interactions by `custom_id`, and the `Guess` button's `custom_id=f"veil_guess:{round_id}"` is stable across rebuilds. The chip buttons have stable custom IDs but no callbacks.

## Tradeoffs

- **One extra Discord API call per guess.** Each guess does an additional `message.edit`. For a single round this is negligible; for very high-velocity bursts (dozens of guesses/second on a popular round) it could brush Discord's per-channel edit budget. Accepted — Veil rounds don't realistically see that traffic, and the failure path (log and swallow) keeps the game flow intact.
...
- A `Veil #N` chip duplicating the embed title — title stays.
- A `New Veil` second action button — separate feature if pursued later.
- An `Unveil` reveal/give-up button — separate feature if pursued later.
