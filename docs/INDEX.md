...
| [todo_spec.md](todo_spec.md) | Server todo: `/todo`, **two** sticky Discord boards — the all-todos board (Add/Complete buttons, auto-updating, bottom-stuck) and the mod chore board (recurring chores only, as a daily done/missed scoreboard with who-ticked-it and streaks) — plus dashboard-configured recurring reminder tasks. Recurring tasks **reset** rather than skipping (2026-08-17): an untouched instance is written off `missed_at` when the next comes due, which is what makes the streaks and the missed record possible. The two boards may never share a channel and the config refuses it with a 409, which now also states what clearing the all-todos board costs. **2026-08-18:** both surfaces resolve "moderator" through `AppContext.is_mod` (configured mod/admin role, or administrator/manage_guild) — the dashboard's `moderator` tier now does too, guild-wide — and a chore created after its own time of day spawns today's instance immediately. Corrected 2026-07-26 — the "Add to Todo" context menu it used to document does not exist in the code |
