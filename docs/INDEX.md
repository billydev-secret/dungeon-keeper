...
| [mention_awards_spec.md](mention_awards_spec.md) | Mention Awards: pays whoever is @-mentioned alongside a configured trigger phrase, for games the bot doesn't host (the member-run Hot Seat rotation). Four levers per rule — channel, phrase, amount, announcer role — at Economy → Mention Awards. Content is matched live and never stored, which is why the historical backfill matches on message *shape* instead |
