...
| [survivor_spec.md](survivor_spec.md) | Survivor: NFL pick'em survival pool — weekly straight-up picks, no team twice, per-game locks, one strike, the Gauntlet late-entry replay, Ghost Streak side-game, Tuesday Reckoning | **In build**, targeting NFL Week 1 (Sept 10 2026). Stages 1–3 shipped 2026-08-17: schema (migration 166), `survivor_service`, dashboard panel + admin API, ESPN ingest, and the member surface (join/pick/status/board with per-game locks). Settle engine, Reckoning, gauntlet, ghosts (§2.5, §4.2, stages 4–6) not yet built. Reclassify to Reference when a season can run; plan in [plans/survivor.md](plans/survivor.md) |
...
|---|---|---|
| [plans/survivor.md](plans/survivor.md) | Survivor staging plan: three tiers against the Sept 10 2026 NFL Week 1 deadline, cut lines if a stage slips, and the house obligations (data register, economy ledger kinds, guild-local scheduling) tracked per stage | Stage 1 built 2026-08-17; stages 2–6a to go for Week 1 |
