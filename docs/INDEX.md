...
|---|---|---|
| [plans/config-routes-simplify.md](plans/config-routes-simplify.md) | `/simplify` pass over `routes/config.py` (5,157 lines — largest file in the repo): six stages of behaviour-neutral dedupe, the games-routes coverage gap, and two measured efficiency fixes. Second per-area pass after `economy_cog.py` | Stage 1 in progress 2026-07-30. Records what was deliberately **not** changed (400-vs-422 bounds checks are load-bearing) and the Tier D backlog: package split, unifying with the already-existing `settings_registry.py`, and 22 cog-private imports |
