...
| [nsfw_classifier_spec.md](nsfw_classifier_spec.md) | Shared image classifier: **Marqo** decides the verdict everywhere, NudeNet only tags in age-gated channels. Three-valued verdict (explicit / not / **unknown**, never collapsed), per-consumer failure direction and thresholds, attachments-only scope, metrics for age-gated channels only (with an opt-in pass that records **every** image there, not just the ones a gate judged), and a blocked-images record covering every channel |
