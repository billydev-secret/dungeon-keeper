...
- **Bot-managed roles** — One page listing every role the bot makes for itself (sixteen of them, across fourteen settings and three mechanisms), with each one's live state: in use, never made, deleted, sitting too high in the role list to be handed out, renamed, duplicated, or inherited from another server's settings. Make one now, point a setting at a role of your own, or stop managing one — which stops the bot pointing at a role and never deletes it.
