...
Crossing level 5 (the configured role-grant level) grants the level-5 role and posts an announcement to the level-5 log channel. This post doubles as a promotion-review signal for staff, so it also shows a "Spicy access" field (✅/❌) when the guild has an NSFW grant role configured, and it's held back — not skipped — if the member joined fewer than 2 days ago: `handle_level_progress` parks it in `pending_promotion_posts` instead of sending, and a background sweep (`promotion_review_recheck_loop`, ~30 min cadence) posts it once tenure clears or drops the row if the member has since left. Intermediate level-ups post to the level-up log channel (one embed per crossed level) and are not tenure-gated. Bulk backfill awards grant the role but skip the announcement embed.
...
One internal knob — the voice-tick poll period — is pinned in code and not exposed. The level-5 promotion post's 2-day tenure minimum is likewise pinned. The "Spicy access" field's grant role is the Promotion Review Grant Role named above when set, falling back to the grant whose internal key is literally `nsfw` (see `role_grant_spec.md`) when it isn't.

**The role-grant level is pinned at 5 by decision, not by omission.** `XpSettings.role_grant_level` exists and every enforcement site reads it — the grant, the milestone embed, the promotion-review card, the time-to-level report — so the field sits one line away from being a coefficient, and it is deliberately left out of the two name lists a dial would have to join: `load_xp_settings`' `_FLOAT_COEFFS`/`_INT_COEFFS` (the read path) and `_XP_COEFF_READERS` in `web_server/routes/config.py` (the dashboard's coefficient table, which the write path iterates too). The reason is wording a dial cannot follow: the number is baked into the frozen `time-to-level5` route id and its whole panel (title, subtitle, empty state, chart heading), into the XP panel's *Level 5 Role* label and the *Level-Up Log Channel* hint that tells admins where level 5 is announced instead, and into the Promotion Reviews copy and `manual.html`. A guild that set it to 7 would be granted at 7 while every surface it reads still said *Level 5* — a worse lie than a fixed number. Revisit only together with renaming those surfaces to "the milestone level". `tests/test_xp_system.py::test_role_grant_level_is_pinned_and_not_loadable_from_config` fails if the key is ever wired up without that pass. (2026-08-29 config audit finding #89; closed as won't-do 2026-09-10.)
