...
**The role-grant level is pinned at 5 by decision, not by omission.** `XpSettings.role_grant_level` exists and every enforcement site reads it — the grant, the milestone embed, the promotion-review card, the time-to-level report — so the field sits one line away from being a coefficient, and it is deliberately left out of `load_xp_settings`' coefficient lists. The reason is wording a dial cannot follow: the number is baked into the frozen `time-to-level5` route id and its whole panel (title, subtitle, empty state, chart heading), into the XP panel's *Level 5 Role* label and the *Level-Up Log Channel* hint that tells admins where level 5 is announced instead, and into the Promotion Reviews copy and `manual.html`. A guild that set it to 7 would be granted at 7 while every surface it reads still said *Level 5* — a worse lie than a fixed number. Revisit only together with renaming those surfaces to "the milestone level". `tests/test_xp_system.py::test_role_grant_level_is_pinned_and_not_loadable_from_config` fails if the key is ever wired up without that pass. (2026-08-29 config audit finding #89; closed as won't-do 2026-09-10.)
