...
The preflight guards **every door into a launch, not just the slash command**. The dashboard scheduler re-checks the enabled dial itself; a **feature-rotation** launch (`feature_rotation_service.start_room_game` → `_launch`) reads `check_game_enabled` against the base game type (`SCHEDULE_BASE_GAME_TYPE`, so a display variant honours its base game's toggle) and logs-and-skips a disabled game; and the recap cards' **Play Again** (Clapback) and **Run Again** (Name Your Price, Mt. Rushmore Draft) buttons go through `game_manager.relaunch_refusal`, which runs the two checks the slash entry runs — allowed channel and enabled dial, in that entry's own copy, the game named from `GAME_NAMES` — **plus** a busy check (no game already running in the channel) that the slash entries do not yet have, so a refused press reads like a refused command and a busy channel is refused at this door only until platform-28 / common-lib A.1 unify the two. Until 2026-09-02 those two doors had no guard, and a host could keep a game an admin had just switched off alive from the recap card indefinitely (`docs/reviews/2026-09-02-games-deep-review-findings.md`, safety-sweep-3/4, platform-28).
