...
**`gamebot` is live** (renamed 2026-07-25 from `gamebot_cah`), covering three sub-games that share one Discord bot account: Cards Against Humanity, Connect 4 and Anagrams. Every *Game over!* goes through the single `_pay_gamebot_game` entry point, which reconstructs the game's window (`current_game_window`) and identifies the sub-game (`identify_game`) before dispatching:
...
Six games open a join lobby and wait for a human to press the start button — **Clapback, Spin the Compliment, Marry-Fornicate-Kiss, Most Likely To, Mt. Rushmore Draft, Story Builder** (`LOBBY_GAME_TYPES` in `games/constants.py`). Only these take `start_in:[1-60]`, because only these have a button for a countdown to point at; every other party game posts its first prompt the instant the command runs. `start_in` stamps a `start_epoch` in the game payload and renders it as a live Discord relative timestamp in an **⏰ Starting** field on the lobby embed, refreshed on every join/leave edit. It is **advertising, not automation** — the game does not auto-start, and the host still presses the button.
...
| External tracking watches | unset | One or more (bot, channel, kind) pairs whose result messages are banked (`/games track`); the same bot may appear in several channels |
...
- The external-tracking watch cache (`guild → {(bot, channel) → kind}`) is warmed on load and kept in sync by the `/games track` commands so the `on_message` hot path never touches the DB. Keyed on the pair, not the bot, so a bot playing in several channels is matched in all of them.
