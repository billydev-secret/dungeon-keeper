...
- **Per-guild per-game enable/disable** (`games_game_config`, default enabled).
  Checked by `check_game_enabled` at the start of every game's command **and**
  by the scheduler before each auto-launch. Set on **Games Global Config →
  Available on This Server**, which lists every type in `ALL_GAME_TYPES` (the
  games with their own settings page repeat the same switch there; Photo
  Challenge's lives on its own page instead). Before 2026-08-29 seven games
  could not be switched off at all: `mfk`, `compliment`, `ttl`, `hottakes`,
  `story` and `fantasies` had no panel and never called the check, and
  `legitlibs` was missing from `ALL_GAME_TYPES` entirely so its PUT 404'd. The
  list also carried a phantom `risky_roller`; the bot has only ever called that
  game `risky_roll`, so a row written under the phantom name was read by
  nothing — the entry was dropped rather than given a switch (Risky Rolls is
