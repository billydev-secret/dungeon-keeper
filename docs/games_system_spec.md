...
| `/games track watch <channel> <bot> <kind>` | Mod/Admin | Watch a channel + bot and start banking its game-result messages. `kind` (`Gamebot (Cards Against Humanity + Connect 4)` \| `Cat Bot`) selects the parser/payout. Several bots can be tracked per guild (one watch per bot) |
...
`/games track *` is a format-agnostic collector: an `on_message`/`on_message_edit` listener banks every message from a watched channel+bot RAW (keyed on message id, de-duplicated across restarts/edits) into `games_external_messages`. Nothing is parsed at ingest — metrics are derived later, so a format change never loses history. Each watch carries a `kind` (migration 097 generalised the one-bot-per-guild table to multiple `(channel, bot, kind)` rows, `UNIQUE(guild_id, bot_user_id)`) that selects a parser (`games_external/parser.py`). Per the `docs/plans/external-game-economy.md` plan: **`gamebot` is live** (renamed 2026-07-25 from `gamebot_cah` once Connect 4 turned up posting to the same watched channel — one Discord bot account can't hold two watch rows under `UNIQUE(guild_id, bot_user_id)`, so one `kind` now covers every Gamebot sub-game tracked, and the cog tells them apart per terminal message: `parser.is_game_over` for CAH's "is the winner" phrasing, else Connect 4). When a *Game over!* embed is captured: for **CAH**, `_pay_cah_game` reconstructs the game's final scores (last *Current Standings* embed, folding in submission-only/winner-only players at 0) and calls `pay_cah_game_by_score` — the top scorer earns `EconSettings.reward_cah_win_max` (default 50, dashboard-configurable), everyone else a ratio of it, still firing the same `party_game`/`game_win` triggers. For **Connect 4**, `_pay_connect4_game` reconstructs the roster (the start embed's **Joined Players** field / *Time's up!* recap) and winner (`<@id> has won!`), then calls the flat `pay_game_rewards` (no score to scale by — win/lose only), same as a native party game. Both are paid once via a `games_external_payouts` ledger (migration 099) keyed on the game-over message id, independent of `parse_status` (which edit-recapture resets). **`catbot` is also live**: Cat Bot catches are parsed from message content (`{username} cought <:raritycat:…>`, reverse cats un-reversed, "blessed…doubled" ×2), the catcher resolved by username→member (`get_member_named`), and `pay_cat_catch` credits rarity-tiered coins (common 1 → divine 300) plus the `cat_catch` quest trigger — paid once via the same ledger, keyed on the catch message id.
