...
`POST /api/config/inactive/preview` (admin) answers "who would lose roles if I switched this on", without writing anything or creating the `@Inactive` role. It selects through `compute_candidates` with `cap=UNCAPPED`, so it lists **every** eligible member rather than one run's worth, and returns the saved `sweep_cap` separately — the panel says how many of them a single run would reach, which is how the overflow contract is honored when nothing is truncated. Member/role data is gateway state, so it 503s when the bot isn't connected. Every id is serialized as a string.
...
- **`blocked`** — candidates whose top role is at or above the bot's. `apply_inactive` would raise `Forbidden` on the strip and the loop counts that as a silent non-move, so they are split out and the headline count excludes them.
