...
The purge implementation, `purge_user_data` (`src/bot_modules/services/privacy_service.py`), is deliberately **unwired** rather than deleted. It is the out-of-band path for a genuine legal erasure request (e.g. GDPR): an operator runs it manually against the database. It removes the user's rows from XP, voice sessions, member activity, quality-score history, gender, member events, the interaction graph (both directions), wellness state and counters, usage telemetry (`usage_events` — see [[usage-telemetry-spec]]; nothing prunes that table on a schedule, so this run is the only thing that ever clears it), and audit-event tables; with `keep_messages=False` it also drops the `messages` archive and its children (attachments, mentions, embeds, reactions, sentiment, and the per-user dedup table). Wellness tables vary by guild deployment age; missing tables are tolerated — a missing-table error logs a warning and the rest of the purge proceeds.
