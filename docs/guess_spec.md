# Guess — Dungeon Keeper Module Spec
...
- **Guess role**: Per-server role gating consent. Only members with this role can be submitters or appear as guess-targets in the autocomplete. Anyone in the server can guess.
...
- `guess_role_id` (required) — the consent/eligibility role
- `guess_channel_id` (required) — where crops post and modals are launched from
...
Stored in existing Dungeon Keeper guild config table; namespace keys under `guess.*`.

## Slash Commands

### `/guess submit <image: attachment>`
- **Permission**: Guess role required. Reject with ephemeral error otherwise.
- **Channel**: Must be invoked in `guess_channel_id` (or DM-tolerant; ephemeral notice if elsewhere).
...
- **Flow**:
  1. Defer ephemerally.
  2. Download attachment; persist the original to `./guess_cache/orig/<round_id><ext>` (deleted on first solve — see Storage).
  3. Run crop pipeline (see Crop Pipeline below).
  4. Post crop publicly to `guess_channel_id` with a "Guess" button.
  5. Persist round row.
...
### `/guess optin` / `/guess optout`
- Toggles the Guess role on the invoker. `optin` is a confirmation modal: "By opting in, you consent to (a) submitting NSFW images of yourself for cropping and (b) being a possible answer in the guessing autocomplete. You can opt out anytime." Yes/No buttons.
...
- Use a Discord **string select menu** (the dropdown component) populated dynamically with members holding the Guess role. Discord caps select options at 25, so paginate with prev/next buttons. Pre-filtered to exclude `answer_optout=true` users at the time of click (live snapshot, not round-creation snapshot — this is fine because opted-out users were already excluded).
...
> **Status: retired.** The `/guess submit` `allow_reuse` flag and the four `reuse_*` config keys are no longer surfaced; no scheduler runs. The section below is kept for archival reference only — none of the eligibility, trigger, or stats rules below execute today.
...
- Submitter still holds the Guess role (consent is current)
...
1. Check time since last *new* submission posted to `guess_channel_id`. If less than `reuse_quiet_hours`, skip.
...
6. Post the crop to `guess_channel_id` with a "Guess" button. Embed footer: "🔁 Throwback round — this crop has appeared before."
...
### Mod controls
- `/guess reuse_pause` — pause reuse system for this guild without changing config.
- `/guess reuse_now` — mod-only. Force a reuse post immediately, ignoring the quiet-hours and post-interval gates. Useful for testing.
- `/guess reuse_block <round_id>` — mark a specific original round ineligible for future reuse (e.g. submitter quietly asked, mod judgment).

## Storage

### Image storage
- Original image: written to `./guess_cache/orig/<round_id><ext>` on submit and **deleted as soon as the round is solved** (immediately after the spoilered reveal is posted). Path is recorded in `guess_rounds.original_path` and cleared at unlink time. While unsolved, the bytes stay on disk so the bot can attach them to the reveal even after a restart.
- Cropped image: written to a private cache dir (`./guess_cache/<round_id>.jpg`) so re-posts/edits work; deleted on round deletion.
...
guess_config
  guild_id INTEGER PK
  guess_role_id INTEGER
  guess_channel_id INTEGER
...
Indexes: `guess_guesses(round_id)`, `guess_guesses(guesser_id)`, `guess_rounds(guild_id, created_at)`, `guess_rounds(submitter_id)`, `guess_rounds(guild_id, allow_reuse, solved_at, reuse_blocked)` (composite for the reuse eligibility query).
...
```
dungeon_keeper/cogs/guess/
  __init__.py
  cog.py                # main GuessCog, command registration
  commands/
    submit.py
    guess.py
    stats.py
...
### Submission
- [ ] Non-Guess-role member invoking `/guess submit` gets ephemeral rejection.
...
- [ ] Successful submission posts a publicly visible cropped image in `guess_channel_id`.
...
### Guessing
- [ ] Guess button opens a dropdown populated only with current Guess-role-holders excluding `answer_optout=true` users.
...
### Stats
- [ ] `/guess stats` returns accurate counts for both submitter and guesser views.
- [ ] `/guess leaderboard` for each category returns correctly ranked top 10.
...
- [ ] Submitter losing the Guess role makes all their crops immediately ineligible for reuse.
- [ ] `/guess reuse_block <round_id>` prevents that round's crop from being picked again.
- [ ] `/guess reuse_now` (mod) triggers a reuse post immediately, bypassing time gates.
...
1. **Unit**: crop pipeline stages with fixture images (synthetic + a small curated NSFW test set kept out of repo, loaded from `GUESS_TEST_IMAGES_DIR` env var).
2. **Integration**: full submit→post→guess→solve flow against mocked Discord client.
3. **Second-server**: deploy to test guild, exercise all commands with a small Guess-roled test cohort.
4. **Production smoke**: post-deploy, run `/guess submit` with a known-good fixture and verify channel post.
...
- Image moderation beyond NudeNet's existing labels (e.g. age verification — out of scope, rely on Discord ToS + Guess-role gating + optin consent).
...
3. **Cache cleanup policy**: cron-style cleanup of `guess_cache/` for rounds older than N days where the Discord message still exists (CDN URL takes over)? Or never clean? Suggest: clean at 30 days, document the tradeoff (re-edits to old messages will lose the local crop).
