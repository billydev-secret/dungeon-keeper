...
| pen_pals_optouts | Pen Pals | members who asked not to be matched (`guild_id`, `user_id`, `at`); no content | until the member rejoins (they clear it themselves by joining) | **DECIDED — preserve**. A suppression record, on the same Art 17(3) footing as a no-contact order: its only purpose is to stop the member being processed, so deleting it would cause the very thing they objected to. The row above preserves `pen_pals_pool` *and* `pen_pals_sessions`, so a purged member can still be pooled or in a live session that re-pools them at expiry — clearing this on the way past would silently re-enrol someone who asked to be left out | — | `user_id` already in `SUBJECT_ID_COLUMNS`, so the export discloses it despite the retention ✓; the member clears it themselves at any time by rejoining, which is the only thing that clears it |
