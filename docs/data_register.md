...
| pen_pals_optouts | Pen Pals | members who asked not to be matched (`guild_id`, `user_id`, `at`); no content | until the member rejoins (they clear it themselves by joining) | **YES — purge** (migration 173). A bare preference: only the two requeue paths and `_eligible_pool` read it, and an erasure already prunes `pen_pals_pool`, so nothing is left for it to protect — no Art 17(3) ground to hold it | — | `user_id` already in `SUBJECT_ID_COLUMNS`, so export sees it ✓; note the **preserved** `pen_pals_sessions` (row above) means an erasure mid-session can still see that session expire and re-pool the member — pre-existing, tracked separately |
