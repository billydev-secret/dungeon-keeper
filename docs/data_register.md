...
| mahjong_results | Meadow Mahjong | one row per settled hand: winner, line, how won, stake | indefinite (group game history) | **Anonymised, not deleted**: `winner_id` → NULL on purge. The row also carries the other seats' hand history through `mahjong_result_seats`, so deleting it would erase their records with the requester's | — | `winner_id` in `SUBJECT_ID_COLUMNS` ✓ |
| mahjong_result_seats | Meadow Mahjong | per-seat coin/point deltas + fallow flag of each settled hand | indefinite | **YES — purge** (member's own rows delete; other seats' rows survive on the anonymised result). No Art 17(3) ground — money history is `econ_ledger`'s job | — | `user_id` in `SUBJECT_ID_COLUMNS` ✓; guild_id denormalized ✓ |
