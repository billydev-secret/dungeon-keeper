...
| econ_shop_purchases | Economy — Custom shop items | one member's order of an admin-defined item: `user_id`, the item, the price snapshot at purchase, an optional free-text `note` the member typed ("engrave BILLY"), `resolver_id` naming the mod who delivered or refused it, and a `deny_reason` a mod wrote | indefinite | **YES — purge** (migration 179; added to `economy_service._PURGE_USER_ID_TABLES`, so it rides the existing `econ_purge_user` sweep). No Art 17(3) ground is claimed: the money that moved lives on in the preserved `econ_ledger`, so deleting the order corrupts no balance. Purging is also what keeps the `todos` promise above honest — the buyer's identity must disappear from the mods' board, and the join is the only place it lives | — | `user_id` in `SUBJECT_ID_COLUMNS` ✓; `guild_id` on the row ✓. `resolver_id` is a mod acting in role, cleared with the row it sits on |
