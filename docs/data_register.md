...
|---|---|---|---|---|---|---|
| econ_wallets, econ_ledger, econ_* (49 tables) | Economy | user_id + amounts + timestamps; meta sampled clean (no PII) | indefinite | YES — **`econ_purge_user` shipped `6ac71558`** (`economy_service.py:1304`, called at `privacy_service.py:201`); `econ_ledger` deliberately preserved | — | G1 closed |
| econ_login_digest_cards | Economy — live login digest | `user_id` + the **Discord DM channel id and message id** of that member's morning digest DM, the guild-local day it belongs to, a hash of what the card currently renders, and the seven `LoginOutcome` scalars (paid, streak, milestone, grace, reset, shield, prior streak) frozen at send time. Migration 190. **No rendered prose is stored** — the quest board, the community movers and the wellness blurb are all re-read from source on each refresh, so the wellness section's text never lands here | one row per member per guild, overwritten by the next day's digest; the hourly sweep also deletes any row whose `local_day` isn't today, so it self-prunes within a day | **YES — purge** (in `econ_purge_user`'s `_PURGE_USER_ID_TABLES`). No Art 17(3) ground and none needed: it is a pointer to a message in the member's own DMs, not a record of anything. Erasing the row stops the card being edited; the DM itself is the member's own copy in their own inbox, which they can delete, and which the bot does not reach into — consistent with every other DM the bot has sent them | — | `user_id` in `SUBJECT_ID_COLUMNS` ✓; guild_id on the row ✓. The stored ids are Discord snowflakes, so any dashboard surface would need the string treatment — there is none today |
...
| quality_score_leaves | Orphaned | none | indefinite | YES | — | empty; its leave-of-absence writer went 2026-07-28 and the quality score it served went 2026-08-31. No reader remains — drop needs a migration and sign-off |
| usage_events | Telemetry | command/panel usage per user | indefinite (no pruning, by design) | YES | — | spec discloses |
| ping_events | Ping Response report | **the pinger, not the pinged**: `author_id` of whoever sent a role ping, plus channel, role ids, `@everyone` flag and timestamp. No content, and **no responder rows at all** — turnout is counted at read time from `messages` and `reaction_log` and only ever surfaces as a number | indefinite | **YES — purge** (migration 198; `privacy_service.py`, delete by `guild_id`+`author_id`). No Art 17(3) ground worth claiming: it is analytics about how a ping landed, and "we wanted the numbers" is not a ground | — | `author_id` in `SUBJECT_ID_COLUMNS` ✓ so the export sees it. `role_ids` is a JSON list of **role** ids, not member ids, so it is not a `LIST_VALUED_MEMBER_COLUMNS` blind spot. Deliberately stores no per-responder rows — "who personally answered which ping" was scoped out as a heavier footprint than the question needed. Disclosed in manual.html §Your Data & Privacy |
...
| wellness_* (12 tables) | Wellness | usage caps, streaks, blackouts | 30d post-optout GC (wired ✓) | YES | local Ollama only | model citizen; §8 reconciled; partners table dropped by migration 189 (never used) |
...
| confession_pending | Confessions | **confession body + author_id** — the only place the bot stores a confession's text itself (`anon_audit_log` stores none and joins `messages`); written only while mod-approve mode holds a submission | deleted on approve, on reject, and by a **7-day sweep** (migration 200). No rejected-confession archive of any kind | **YES — purge** (`privacy_service.py`, confessions block). Deleted outright rather than left to the TTL its sibling `confession_threads` relies on: the row is queued *to be published*, so erasing a member and then posting what they wrote — because a mod reached the queue before the sweep — is not defensible | — | `author_id` in `SUBJECT_ID_COLUMNS` ✓ so export sees it; guild_id present ✓. Not in `THIRD_PARTY_TABLES`: the row names one member and no second party. Seven days is inherited from the manual.html promise that the confession↔author link self-destructs in a week, not chosen independently |
...
| grant_role_permissions | Role Grants | the *Who Can Hand This Out* allow-list per grant: `entity_type` 'user' rows name a **member** in `entity_id` (role rows name a role and are not personal data) | until the admin removes the entry on the Role Grants panel | **NO — not reached by `purge_user_data` today.** An authority grant rather than a record about the member, so it sits with the preserved mod record above; open decision, since a member erased mid-tenure leaves a bare user id on a keeper list with no clean Art 17(3) ground | — | `entity_id` added to `SUBJECT_ID_COLUMNS` 2026-08-29, so an access export now sees it. **Legacy `give_role_permissions` — DROPPED (migration 193, 2026-08-30).** The pre-`grant_role_permissions` shape (`guild_id`, `target_role_id`, `entity_type`, `entity_id`), created by application code that no longer exists. It held 2 prod rows naming 2 members, had no reader or writer anywhere in `src/`, and was created by no migration; the rows were a silent no-op invisible to the Role Grants panel, so an erasure or access request could not meaningfully act on them. Dropped rather than wired up — the live allow-list is `grant_role_permissions` |
...
| econ_theme_submissions | Economy — Flash Themes | one member's purchased themed day: `user_id`, the `title` and `blurb` they wrote, the price snapshot, `resolver_id` naming the mod who approved or declined it, and a `deny_reason` a mod wrote. The `title`/`blurb` are member-authored free text and are **published** — the live announcement is a pinned message in the theme channel crediting the buyer by name | indefinite | **YES — purge, with one anonymised exception** (migration 188; added to `economy_service._PURGE_USER_ID_TABLES`, so it rides the existing `econ_purge_user` sweep). No Art 17(3) ground is claimed: the money that moved lives on in the preserved `econ_ledger`, so deleting the submission corrupts no balance. **A theme that is RUNNING is detached, not deleted** (`anonymise_live_theme`, called at the top of `econ_purge_user` beside `release_open_orders`): a live row holds a pinned announcement in Discord that only the hourly expiry sweep knows how to take down, and that sweep finds its work by reading `state = 'live'` rows. Deleting the row would strip the member's name and simultaneously strand the pin at the top of the channel permanently, with nothing left to say it should ever come down — a worse outcome for the erased member than the row itself. So `user_id` goes to 0 (the same unknown-member every surface renders, and which also takes the row out of the purge's `WHERE user_id = ?` sweep) while the window and pin ids stand, and the announcement comes down on schedule. The `todos` anonymise-don't-delete precedent above | — | `user_id` in `SUBJECT_ID_COLUMNS` ✓; `guild_id` on the row ✓. `resolver_id` is a mod acting in role, cleared with the row it sits on. **The published announcement itself is not erased by the purge** — it is an ordinary Discord message that still carries the buyer's `<@id>` mention, and it comes down with the pin when the window ends (up to `MAX_THEME_HOURS` = 168h later). The purge is a synchronous DB path with no gateway access, so it cannot edit the embed; a mod ends the theme early on request, which deletes the post immediately. The manual's privacy notice states this explicitly rather than implying the live post is anonymised |
