...
| A4 | Show the top 3 (**coach shows 1** — revised on live QA 2026-08-23: three targets plus gaps plus dead weight plus a suggestion was a wall of text mid-turn; `ASSIST_SHOWN` is the single source of truth, shared with `suggest_discard`'s intersection so the suggestion can never contradict the shown hand); tie-break distance → value descending → card order | Distances cluster hard (a sampled 13-tile rack scored 8, 9, 10, 10, 10), so ties are the common case, not the edge case. Without a deterministic tie-break the readout would reshuffle between renders of an unchanged rack. |
...
| A12 | Coach judges the **live discard** too — win / call / let it go — using `match_logic.call_advice`, the same function the bot brain plays on (live QA 2026-08-23) | The claim window is 6–8 s and it is the sharpest decision in the game; a coach that names a discard to throw but stays silent when a hand-winning tile lands is advising the easy half. Extracting the judgement out of `bot_logic` and into `match_logic` also removed the duplicate — and dodged the `game_logic → bot_logic` import cycle a copy would have needed. |
