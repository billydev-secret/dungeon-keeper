                ...
                + _fmt(
                    [
                        ...
                        (
                            ...
                            "All members inactive for a period. Add `channel:` or `exclude_gif_only:True`.",
                        ),
                        (
                            "/report oldest_sfw count:10",
                            "Members without NSFW access, ranked by longest silence.",
                        ),
                        (
                            "/xp_level_review level:5",
                            "How long does it take to reach a level? Histogram with stats.",
                        ),
                        (
                            "/purge count:50",
                        ...
                        (
                            "/dropoff period:week",
                            "Who disengaged the most? Compares two consecutive windows. "
                            "Add `member:@user` for a full profile instead.",
                        ),
                        (
                            "/burst_ranking limit:5",
                            "Who sparks conversation when they return vs. who posts quietly?",
                        ),
                        (
                            "/interaction_heatmap timescale:week",
                            "Matrix heatmap of who talks to whom (replies + mentions).",
                        ),
                        (
                            "/report role_growth resolution:week",
                            "Cumulative role grants over time. Add `roles:Name1,Name2` to filter.",
                        ),
                        (
                            "/report promotion_review",
                            "Members past level 5 who still lack NSFW access.",
                        ),
                        (
                            "/report message_cadence resolution:day",
                            "Time between messages as a candlestick chart. Green = speeding up.",
                        ),
                        (
                            "/report join_times resolution:hour_of_day",
                            "When do new members join? By hour or day of week.",
                        ),
                        (
                            "/report quality_scores",
                            "Ranked quality scores: engagement, consistency, resonance, activity.",
                        ),
                        (
                            "/report message_rate days:7",
                            "When is the server busy? Messages per 10-min slot, averaged.",
                        ),
                        (
                            "/report greeter_response days:30",
                            "How long do new members wait for a greeter hello?",
                        ),
                        (
                            "/report nsfw_gender resolution:week",
                            "NSFW posting by gender. `display:bar` or `display:line`.",
                        ),
                        (
                            "/report backfill_roles",
                            "Sync role events with current state. Run after bulk role edits.",
                        ),
                    ]
                ),
            )
        ...
        pages.append(
            _page(
                ...
                + _fmt(
                    [
                        (
                            "/activity resolution:day mode:xp",
                            "Bar chart of messages or XP over time. "
                            "Scope with `member:` or `channel:`. "
                            "Resolutions: hour, day, week, month, hour_of_day, day_of_week.",
                        ),
                        (
                            "/session_burst member:@user",
                            "How active is someone in the hour after a break? "
                            "Shows whether they spark conversation or post quietly.",
                        ),
                        (
                            "/connection_web timescale:week",
                            "Network graph of replies and mentions. "
                            "Add `member:@user` to focus. Key options: `min_pct`, `layers`, `limit`.",
                        ),
                        (
                            "/chilling_effect lookback_days:30",
                            "Who makes others go quiet when they arrive? Correlation-based.",
                        ),
                        (
                            "/invite_web member:@user",
                            "Who invited whom, as a network graph.",
                        ),
                    ]
                ),
            )
                ...
                + _fmt(
                    [
                        (
                            ...
                            "Randomize nicknames for active members. `action:restore` to undo.",
                        ),
                        (
                            "/foolsday_exclude user:@User",
                            "Exclude a member from the shuffle.",
                        ),
                        (
                            "/foolsday_join user:@User",
                            "Add a member to the active shuffle.",
                        ),
                        (
                            "/foolsday_include user",
                            "Remove someone from the exclusion list.",
                        ),
                        ("/foolsday_exclusions", "List excluded members."),
                        (
                            "/foolsday_samename name",
                            "Give everyone the same name (random or custom). Restore to undo.",
                        ),
                        (
                            "/foolsday_repair",
                            "Fix broken mappings by walking through each one.",
                        ),
                    ]
                ),
            )
                ...
                + _fmt(
                    [
                        (
                            ...
                            "Backfill the interaction graph from message history. "
                            "Run once after setup. `reset:True` clears old data first.",
                        ),
                        (
                            "/gender set member:@user gender:Female",
                            "Tag a member as Male, Female, or Non-binary for NSFW analytics.",
                        ),
                        (
                            "/gender check member:@user",
                            "See a member's current gender tag.",
                        ),
                        (
                            "/gender classify",
                            "Step through unclassified members one by one with buttons.",
                        ),
                    ]
                ),
            )
                ...
                + _fmt(
                    [
                        (
                            ...
                            "AI flags rule violations and patterns in a member's recent messages.",
                        ),
                        (
                            "/ai scan count:50",
                            "AI scans the last N messages in this channel for rule violations.",
                        ),
                        (
                            "/ai channel question:... minutes:60",
                            "Ask the AI about a channel's recent activity. "
                            "e.g. *'Did anyone harass a new member?'*",
                        ),
                        (
                            "/ai query member:@user question:...",
                            "Ask the AI about a member's message history. "
                            "e.g. *'Has this person been hostile toward newcomers?'*",
                        ),
                    ]
                ),
            )
                ...
                + _fmt(
                    [
                        (
                            ...
                            "Show which channels have XP turned off.",
                        ),
                        (
                            "/xp_backfill_history days:30",
                            "Scan past messages to fill XP gaps.",
                        ),
                        ("/welcome_preview", "Preview the welcome message."),
                        ("/leave_preview", "Preview the leave message."),
                        (
                            "/inactivity_prune status",
                            "Current prune rule, threshold, and exemptions.",
                        ),
                        (
                            "/inactivity_prune exempt member:@user",
                            "Protect a member from pruning.",
                        ),
                        (
                            "/inactivity_prune unexempt member:@user",
                            "Remove a prune exemption.",
                        ),
                        (
                            "/inactivity_prune run",
                            "Run a prune now instead of waiting for the daily schedule.",
                        ),
                        (
                            "/auto_delete del_age:30d run:1d",
                            "Delete old messages, optionally on a schedule. `run:once` or `run:off`.",
                        ),
                        (
                            "/auto_delete_configs",
                            "View and manage all auto-delete schedules.",
                        ),
                        (
                            "/quality_leave add member:@user days:30",
                            "Put a member on leave (pauses quality scoring).",
                        ),
                        (
                            "/quality_leave remove member:@user",
                            "End a member's leave of absence.",
                        ),
