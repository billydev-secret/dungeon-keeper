...
def register_foolsday_commands(bot: "Bot", ctx: "AppContext") -> None:
    ...
    @bot.tree.command(
        ...
        description="Exclude a user from the April Fools name shuffle.",
    )
    @app_commands.describe(user="The member to exclude from shuffling.")
    async def foolsday_exclude(interaction: discord.Interaction, user: discord.Member) -> None:
        if not ctx.is_mod(interaction):
            await interaction.response.send_message(
                "You don't have permission to use this command.", ephemeral=True
            )
        ...
        with ctx.open_db() as conn:
            ...
            saved = _load_names(conn, guild.id)
            original_name = saved.get(user.id)
            if original_name is not None:
                current_nick = user.display_name

                # Restore the excluded user to their original name
                nick = None if original_name == user.name else original_name
                try:
                    log.info("Foolsday exclude: restoring %s (%d) to %r", user, user.id, original_name)
                    await user.edit(nick=nick, reason="April Fools — excluded, restoring original name")
                    details.append(f"Restored {user.mention} to their original name.")
                except (discord.Forbidden, discord.HTTPException) as exc:
                    log.warning("Foolsday exclude: could not restore %s (%d): %s", user, user.id, exc)
                    details.append(f"Could not restore {user.mention}'s name: {exc}")
        ...
        log.info("Foolsday: %s excluded %s (%d)", interaction.user, user, user.id)
        msg = f"{user.mention} excluded from foolsday shuffle."
        if details:
            msg += "\n" + "\n".join(details)
        await interaction.followup.send(msg, ephemeral=True)
