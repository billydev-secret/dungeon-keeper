"""Async SQLite helpers for Musical Chairs. All SQL lives here."""
...
async def get_game(db: GamesDb, game_id: int) -> MusicalChairsGame | None:
    row = await db.fetchone("SELECT * FROM mc_games WHERE id = ?", (game_id,))
...
async def fetch_active_games(db: GamesDb) -> list[MusicalChairsGame]:
    rows = await db.fetchall("SELECT * FROM mc_games WHERE state = 'ACTIVE'")
    return [game_from_row(r) for r in rows]


async def fetch_lobby_games(db: GamesDb) -> list[MusicalChairsGame]:
    rows = await db.fetchall("SELECT * FROM mc_games WHERE state = 'LOBBY'")
    return [game_from_row(r) for r in rows]
...
async def fetch_sweepable_games(db: GamesDb, now: float) -> list[MusicalChairsGame]:
...
async def get_config(db: GamesDb, guild_id: int) -> dict:
    row = await db.fetchone("SELECT * FROM mc_config WHERE guild_id = ?", (guild_id,))
    defaults: dict = {
        "guild_id": guild_id,
        "min_music": 5.0,
        "max_music": 15.0,
...
async def get_stats(db: GamesDb, guild_id: int, user_id: int) -> dict:
    rows = await db.fetchall(
        """
        SELECT roster, winner_id, loser_id FROM mc_games
        WHERE guild_id = ?
