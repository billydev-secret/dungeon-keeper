...
from bot_modules.services.embeds import AUTO_DELETE_PRIMARY
...
from bot_modules.core.utils import format_user_for_log, get_bot_member, get_guild_channel_or_thread
...
class AutoDeleteCog(commands.Cog):
    ...
    @app_commands.default_permissions(manage_guild=True)
    async def auto_delete_configs(self, interaction: discord.Interaction) -> None:
        ctx = self.ctx
        if not ctx.is_mod(interaction):
            await interaction.response.send_message(
                "You don't have permission to use this command.", ephemeral=True
...
async def setup(bot: Bot) -> None:
    await bot.add_cog(AutoDeleteCog(bot, bot.ctx))
