...
TRIGGER_KIND_INFO: dict[str, str] = {
    ...
    "mention_award": "Being @-mentioned alongside a configured trigger phrase in a watched channel — the Mention Awards watcher pays the member tagged. Built for games the bot does not host (the Hot Seat rotation, where the outgoing contestant announces the next one), so the announcement members already post is the payout event. Pays coins directly beside this trigger (the cat_catch pattern). Who may award is the rule's announcer-role lever; with it unset, anyone in the channel can. Event cadence: once per announcement (keyed on the message).",
